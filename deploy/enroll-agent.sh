...
default_client_id() {
  local host
  host="$(hostname -s 2>/dev/null || hostname 2>/dev/null || printf 'vpsman-agent')"
  printf '%s' "$host" | tr -c 'A-Za-z0-9._:-' '-' | sed 's/^-*//; s/-*$//; s/--*/-/g'
}
...
prompt_value VPSMAN_API_URL "Panel/API URL"
prompt_secret VPSMAN_ENROLLMENT_TOKEN "Enrollment token"
prompt_value VPSMAN_CLIENT_ID "Agent client id" "$(default_client_id)"
...
[[ "$VPSMAN_API_URL" == http://* || "$VPSMAN_API_URL" == https://* ]] \
  || die "VPSMAN_API_URL must start with http:// or https://"
...
install -m 0600 "$tmp_dir/agent.toml" "$CONFIG_PATH"
...
log "agent enrollment complete for ${VPSMAN_CLIENT_ID}"
