...
- Keep CPU utilization optional and derived from valid `/proc/stat` deltas;
  ...
  Scope aggregate live rate with `network.rate.interfaces`: an absent or unset
  rule defaults to a live reference to each VPS's current `traffic.selectors`.
  The explicit input syntax `[traffic.selectors]` stores that same typed
  rule-reference object. Empty input or `[]` explicitly selects all collected
  interfaces. Other explicit values reuse the existing selector grammar, but
  live-rate scope is interface-only: any `+rx` or `+tx` suffix is ignored and
  both separately reported directions remain present for each selected
  interface. Direction suffixes continue to control traffic accounting only.
