...
test("captures main console screenshots for regression review", async ({ page }, testInfo) => {
  const reviewRoot = process.env.VPSMAN_SCREENSHOT_REVIEW_DIR ?? testInfo.outputPath("screenshots");
  const projectDir = join(reviewRoot, testInfo.project.name);
  mkdirSync(projectDir, { recursive: true });

  await page.goto("/");
  const views = testInfo.project.name.includes("mobile") ? desktopViews.slice(0, 1) : desktopViews;
  const manifest: Array<Record<string, unknown>> = [];

  for (const entry of views) {
    if (entry.view !== "Fleet") {
      await activate(
        page.getByRole("navigation", { name: "Primary console navigation" }).getByRole("button", {
          name: entry.view,
          exact: true,
        }),
      );
    }
    await expect(page.locator(".consoleHeader").getByRole("heading", { name: entry.heading })).toBeVisible();
    ...
    expect(layout.visibleTextLength, `${entry.view} visible text length`).toBeGreaterThan(200);
    expect(layout.blankMain, `${entry.view} main content should not be blank`).toBe(false);

    const screenshotPath = join(projectDir, `${entry.view.toLowerCase()}-${testInfo.project.name}.png`);
    const screenshot = await page.screenshot({ fullPage: true, path: screenshotPath });
    expect(screenshot.length, `${entry.view} screenshot should not be empty`).toBeGreaterThan(12_000);
  ...
  const manifestPath = join(projectDir, `manifest-${testInfo.project.name}.json`);
  writeFileSync(manifestPath, `${JSON.stringify({ generated_by: "console-screenshot-review", views: manifest }, null, 2)}\n`);
});
