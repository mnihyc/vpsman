...
test("schedule registry lifecycle uses UUID actions from the browser", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name.includes("mobile"), "schedule lifecycle editing is covered in the desktop console");
  ...
  await activate(page.getByRole("button", { name: "Disable edge-health-hourly" }));
  await expect(page.getByText("Confirm schedule disable")).toBeVisible();
  await activate(page.locator(".confirmationPrompt").getByRole("button", { name: "Disable" }));
  await expect(page.locator(".status").filter({ hasText: "disabled" })).toBeVisible();

  await activate(page.getByRole("button", { name: "Enable edge-health-hourly" }));
  await expect(page.getByText("Confirm schedule enable")).toBeVisible();
  await activate(page.locator(".confirmationPrompt").getByRole("button", { name: "Enable" }));
  await expect(page.locator(".status").filter({ hasText: "enabled" }).first()).toBeVisible();

  await activate(page.getByRole("button", { name: "Edit edge-health-hourly" }));
  await expect(page.getByRole("heading", { name: "Modify schedule" })).toBeVisible();
  await page.getByLabel("Schedule name").fill("edge-health-hourly");
  await page.getByLabel("Schedule cron expression").fill("10,40 * * * *");
  await page.getByLabel("Schedule target expression").fill("provider:alpha && country:US");
  ...
  await activate(page.locator(".confirmationPrompt").getByRole("button", { name: "Update schedule" }));
  await expect(page.getByText("10,40 * * * *")).toBeVisible();

  await activate(page.getByRole("button", { name: "Defer edge-health-hourly" }));
  await page.getByLabel("Schedule defer until").fill("2026-06-04T09:30");
  await page.getByLabel("Schedule defer reason").fill("Customer maintenance freeze for APAC packet-filter rollout");
  await activate(page.locator(".inlineOpsForm").getByRole("button", { name: "Defer", exact: true }));
  await expect(page.getByText("Confirm schedule defer")).toBeVisible();
  await expect(page.getByText("Customer maintenance freeze for APAC packet-filter rollout")).toBeVisible();
  await activate(page.locator(".confirmationPrompt").getByRole("button", { name: "Defer" }));
  await expect
    .poll(() =>
      page.evaluate(() => {
        const requests = (window as unknown as {
          __vpsmanTestRequests: { scheduleActions: Array<{ method: string; path: string }> };
  ...
  await activate(page.getByRole("button", { name: "Apply edge-health-hourly now" }));
  await expect(page.getByText("Confirm apply now")).toBeVisible();
  await expect(page.getByText("Dispatches a normal job from the saved schedule without changing the next scheduled run.")).toBeVisible();
  await activate(page.locator(".confirmationPrompt").getByRole("button", { name: "Apply now" }));
  await expect
    .poll(() =>
      page.evaluate(() => {
        const requests = (window as unknown as {
  ...
  await activate(page.getByRole("button", { name: "Delete edge-health-hourly" }));
  await expect(page.getByText("Confirm schedule delete")).toBeVisible();
  await activate(page.locator(".confirmationPrompt").getByRole("button", { name: "Delete" }));
  await expect(page.getByText("edge-health-hourly")).toHaveCount(0);

  const actions = await page.evaluate(() => {
    const requests = (window as unknown as {
      __vpsmanTestRequests: { scheduleActions: Array<{ method: string; path: string; body?: unknown }> };
    }).__vpsmanTestRequests;
...
test("expert operator can scan and dispatch across a realistic 24 VPS fleet", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name.includes("mobile"), "20+ VPS expert audit is a dense desktop workflow");
  ...
  await expect(page.getByRole("heading", { name: "VPS instances" })).toBeVisible();
  const fleetGrid = page.getByLabel("VPS instance records data grid");
  await expect(fleetGrid.getByText("ams-payments-edge-01").first()).toBeVisible();
  ...
  await expect(page.locator(".inspector").getByText("Last seen")).toBeVisible();
  await expect(page.locator(".inspector")).toContainText("acmecloud");
  ...
  await composer.getByLabel("Saved command template").selectOption("46464646-5656-4789-8abc-defdefdefdef");
  await composer.getByLabel("Command argv").fill("/usr/local/bin/check-payment-edge --scope eu-west --json");
  await composer.getByLabel("Bulk target selector expression").fill("provider:acmecloud && tag:payments");
  ...
  await expect(impact.getByText("24 targets / standard dispatch")).toBeVisible();
  await expect(impact.getByText("Stale")).toBeVisible();
  await expect(impact.getByText("Unavailable")).toBeVisible();
  await expect(impact.locator(".targetChip").filter({ hasText: "ams-payments-edge-01" }).first()).toHaveAttribute(
    "title",
    "pay-prod-ams-edge-01",
  );
  ...
  await expect(composer.getByText("24 resolved (21 online, 1 stale, 2 unavailable)")).toBeVisible();
  ...
  await expect(resultPanel.locator(".executionResultStats span").filter({ hasText: "failed" }).filter({ hasText: "2" })).toBeVisible();
  await expect(resultPanel.locator(".executionResultStats span").filter({ hasText: "unavailable" }).filter({ hasText: "2" })).toBeVisible();
  await expect(resultPanel.getByText(/partial success: 20 done, 2 failed, 2 unavailable/)).toBeVisible();
  const failedReasons = resultPanel.getByLabel("Failed target reasons");
  await expect(failedReasons.getByText(/stale: agent rejected shell_argv command_version 3/)).toBeVisible();
  await expect(failedReasons.getByText(/process_guard: permission denied/)).toBeVisible();
  ...
  await inspector.getByLabel("Enrollment default tags").fill("provider:acmecloud,country:SG,payments,edge,env:prod");
  await inspector.getByLabel("Enrollment default display name").fill("sin-payments-edge-25");
  ...
  await expect(inspector.locator(".enrollmentInstallCommand code")).toContainText("VPSMAN_ENROLLMENT_TOKEN='vpsm_provision_token_secret'");
  ...
  await page.screenshot({ fullPage: true, path: testInfo.outputPath("expert-24-vps-dispatch.png") });
});
...
async function installTwentyFourVpsExpertMock(page: Page) {
  await page.addInitScript(() => {
    ...
    const agents = Array.from({ length: 24 }, (_, index) => {
      ...
      const status = index === 7 || index === 8 ? "offline" : index === 9 ? "stale" : "online";
      return {
        capabilities: {
        ...
        stale_reason: status === "stale" ? "agent rejected shell_argv command_version 3" : null,
    ...
    const matchesSelector = (agent: (typeof agents)[number], selector: string) => {
      ...
      return expression
        ...
        .every((term) => {
          ...
          if (!value) {
            return valueMatches(agent.id, kind, true) || valueMatches(agent.display_name, kind, true);
          }
          if (kind === "id") {
    ...
    const resolveTargets = (body: unknown) => {
      const selector = (body as { selector_expression?: string } | null)?.selector_expression ?? "";
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (pathname === "/api/v1/jobs" && method === "POST") {
        ...
        jobTargets[jobId] = targets.map((agent) => ({
          client_id: agent.id,
          completed_at: agent.status === "offline" ? null : "2026-06-07T04:50:10Z",
          exit_code: agent.status === "stale" ? 2 : agent.id === "pay-prod-sfo-edge-11" ? 13 : agent.status === "offline" ? null : 0,
          job_id: jobId,
          message:
            agent.status === "stale"
              ...
              : agent.status === "offline"
                ? "agent offline"
                : "completed",
          started_at: agent.status === "offline" ? null : "2026-06-07T04:50:05Z",
          status: agent.status === "stale" || agent.id === "pay-prod-sfo-edge-11" ? "failed" : agent.status === "offline" ? "dispatch_failed" : "completed",
        }));
        return jsonResponse({ accepted_targets: acceptedTargets.length, job_id: jobId, status: "accepted" });
      }
      const targetsMatch = pathname.match(/^\/api\/v1\/jobs\/([^/]+)\/targets$/);
      ...
      if (jobMatch && method === "GET" && jobMatch[1] === "24242424-7777-4888-9999-aaaaaaaaaaaa") {
        return jsonResponse({
          actor_id: null,
...
async function collectLayoutSignals(page: Page, scopeSelector: string) {
  return page.evaluate((selector) => {
    ...
    const controls = Array.from(scope.querySelectorAll("button, input, select, textarea, summary, .searchExpressionInput")).filter(visible);
    const rects = controls.map((element) => ({ element, rect: element.getBoundingClientRect() }));
    const scopeRect = scope.getBoundingClientRect();
    const clippedControls = rects
      .filter(({ rect }) => rect.right > scopeRect.right + 1 || rect.left < scopeRect.left - 1)
