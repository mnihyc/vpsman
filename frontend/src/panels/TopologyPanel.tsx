...
import { ApiResponseError, apiGet } from "../api";
...
export function TopologyPanel({
  ...
  if (activeSubpage === "port_forwards") {
    const hasNetworkWriteScope = Boolean(
      operator?.scopes.includes("*") ||
      operator?.scopes.includes("network:write"),
    );
    return (
      <PortForwardingPanel
        agents={agents}
      ...
      <section className="fleetPanel networkOverviewHeader">
        <div className="sectionHeader">
          <div>
            ...
            <span>
              Declared tunnel state, runtime evidence, and routing control
            </span>
          </div>
        ...
        >
          ...
          <Metric
            ...
            tone={attention > 0 ? "warning" : "normal"}
            value={attention}
          />
          <Metric
            label="Declared observations"
            value={telemetryTunnels.length}
          />
          <Metric
          ...
          <Metric
            ...
            tone={ospfAttention > 0 ? "warning" : "normal"}
            value={ospfAttention}
          />
        </div>
      </section>
      <section className="fleetPanel networkOverviewActions">
        <div className="sectionHeader">
          <div>
            <h2>Workflows</h2>
            <span>
              Open the evidence or control surface needed for the current task
            </span>
          </div>
        </div>
        <div
        ...
        >
          <WorkflowLink
            ...
            label="Tunnel plans"
            detail="Declare ownership, endpoints, addresses, and optional OSPF"
            onClick={() => onSelectSubpage("tunnel_plans")}
          />
          <WorkflowLink
            icon={<Repeat2 size={18} />}
            label="Port forwarding"
            detail="Manage per-VPS owned DNAT rules and inspect apply state"
            onClick={() => onSelectSubpage("port_forwards")}
          />
          <WorkflowLink
            icon={<GitGraph size={18} />}
            label="Graph"
            detail="Inspect declared relationships and runtime drift"
            onClick={() => onSelectSubpage("graph")}
          />
          <WorkflowLink
            icon={<Activity size={18} />}
            label="Tests"
            detail="Run status, probe, and bounded throughput jobs"
            onClick={() => onSelectSubpage("tests")}
          />
          <WorkflowLink
            icon={<Gauge size={18} />}
            label="OSPF"
            detail="Check bound adapters and control reviewed or automatic cost updates"
            onClick={() => onSelectSubpage("ospf")}
          />
          <WorkflowLink
            icon={<Route size={18} />}
            label="Evidence"
            detail="Review persisted observations, trends, and jobs"
            onClick={() => onSelectSubpage("evidence")}
          />
        </div>
      </section>
    </div>
...
function TunnelPlansWorkspace({
  ...
  onUpdateTunnelPlan: (
    planId: string,
    request: UpdateTunnelPlanRequest,
  ) => Promise<TunnelPlanMutationResponse>;
  onUpdateNetworkAdapterDefinition: (
    definitionId: string,
    request: UpsertNetworkAdapterDefinitionRequest,
  ...
  const runtimeEdgeByPlan = useMemo(
  ...
  const columns = useMemo<ConsoleDataGridColumn<TunnelPlanRecord>[]>(
    () => [
      {
        ...
        cell: (plan) => (
          <span className="historyPrimary">
            ...
            <small title={`${plan.kind} · ${plan.plan.interface_name}`}>
              {formatTunnelKind(plan.kind)} · {plan.plan.interface_name}
            </small>
          </span>
        ),
        mobilePrimary: true,
        searchValue: (plan) =>
          `${plan.name} ${plan.kind} ${plan.plan.interface_name}`,
      ...
      },
      {
        ...
        header: "Endpoints",
        cell: (plan) => {
          const left = clientDisplayNameFromMap(
            plan.left_client_id,
            agentNameById,
          );
          const right = clientDisplayNameFromMap(
            plan.right_client_id,
            agentNameById,
          );
          return (
            <span className="historyPrimary">
              <strong title={`${left}; full ID ${plan.left_client_id}`}>
                {left}
              </strong>
              <small title={`${right}; full ID ${plan.right_client_id}`}>
                {right}
              </small>
            </span>
          );
        ...
        searchValue: (plan) =>
          `${clientDisplayNameFromMap(plan.left_client_id, agentNameById)} ${plan.left_client_id} ${clientDisplayNameFromMap(plan.right_client_id, agentNameById)} ${plan.right_client_id}`,
        sortValue: (plan) =>
          clientDisplayNameFromMap(plan.left_client_id, agentNameById),
        minSize: 180,
        size: 220,
      },
      {
      ...
      },
      {
        ...
        cell: (plan) => (
          <span className="historyPrimary">
            <strong
              title={runtimeManagerLabel(plan.plan.runtime_control?.manager)}
            >
              {runtimeManagerLabel(plan.plan.runtime_control?.manager)}
            </strong>
            <small>{plan.enabled ? "Enabled" : "Disabled"}</small>
          </span>
        ),
        searchValue: (plan) =>
          `${runtimeManagerLabel(plan.plan.runtime_control?.manager)} ${plan.enabled ? "enabled" : "disabled"}`,
        sortValue: (plan) =>
          runtimeManagerLabel(plan.plan.runtime_control?.manager),
        minSize: 160,
        size: 190,
      },
      {
        ...
        cell: (plan) => {
          ...
          const left = tunnelEndpointDisplayState(
            plan.enabled,
            plan.left_runtime_config,
            edge?.left_runtime_state,
          );
          const right = tunnelEndpointDisplayState(
            plan.enabled,
          ...
          return (
            <span className="endpointStatusPair">
              <span
                className={`status ${tunnelEndpointRuntimeStateBadgeClass(left)}`}
                title={tunnelEndpointStateTitle(
                  "Left",
                  plan.left_runtime_config,
                  edge?.left_runtime_reason,
                  edge?.left_observed_at,
                )}
              >
                L {readableTelemetryToken(left)}
              </span>
              <span
                className={`status ${tunnelEndpointRuntimeStateBadgeClass(right)}`}
                title={tunnelEndpointStateTitle(
                  "Right",
                  plan.right_runtime_config,
                  edge?.right_runtime_reason,
                  edge?.right_observed_at,
                )}
              >
                R {readableTelemetryToken(right)}
              </span>
            </span>
          );
        },
        ...
        searchValue: (plan) => {
          const edge = runtimeEdgeByPlan.get(plan.id);
          return `${tunnelEndpointDisplayState(plan.enabled, plan.left_runtime_config, edge?.left_runtime_state)} ${tunnelEndpointDisplayState(plan.enabled, plan.right_runtime_config, edge?.right_runtime_state)}`;
        },
        sortValue: (plan) =>
      ...
      },
      {
        ...
        header: "Connectivity",
        cell: (plan) => {
          const presentation = tunnelConnectivityPresentation(
            plan,
            runtimeEdgeByPlan.get(plan.id),
          );
          return (
            <span className="historyPrimary tunnelConnectivitySummary">
              <strong
                className={`status ${presentation.statusClass}`}
        ...
        sortValue: (plan) =>
          tunnelConnectivityPresentation(plan, runtimeEdgeByPlan.get(plan.id))
            .label,
        minSize: 190,
        size: 230,
      },
        ...
        cell: (plan) => (
          <span className="historyPrimary">
            ...
            <small
              title={
                plan.plan.ospf
                  ? `${formatOspfMode(plan.plan.ospf.mode)} · ${readableTelemetryToken(plan.ospf_status)}`
                  : "Tunnel only"
              }
            >
              {plan.plan.ospf
    ...
    [agentNameById, runtimeEdgeByPlan],
  );

  function cleanupRetryNeeded(plan: TunnelPlanRecord) {
    return (
      !plan.enabled &&
  ...
  function applyRetryNeeded(plan: TunnelPlanRecord) {
    return (
      plan.enabled &&
      [plan.left_runtime_config.status, plan.right_runtime_config.status].some(
        (status) =>
  ...
  const actions: ConsoleDataGridAction<TunnelPlanRecord>[] = [
    {
      description: (rows) =>
        editorOpen
          ? "Close the current tunnel plan editor first."
          : rows.length === 1
            ? `Edit ${rows[0].name}.`
            : "Select one tunnel plan to edit.",
      disabled: (rows) => pending || editorOpen || rows.length !== 1,
      icon: <Pencil size={14} />,
      label: "Edit",
      onSelect: (rows) => {
        if (!rows[0]) return;
        setEditingPlan(rows[0]);
        setCreateOpen(true);
      },
    },
    {
      description: (rows) =>
        rows.length === 1
          ? `Export ${rows[0].name} as JSON.`
          : "Select one tunnel plan to export.",
      disabled: (rows) => pending || rows.length !== 1,
      icon: <Download size={14} />,
      label: "Export",
      onSelect: (rows) => rows[0] && void exportPlan(rows[0]),
    },
    {
      description: (rows) => {
        ...
        return eligible.length > 0
          ...
          : "All selected tunnel plans are already enabled.";
      },
      disabled: (rows) => pending || rows.every((plan) => plan.enabled),
      icon: <Power size={14} />,
      label: "Enable",
      onSelect: (rows) =>
        requestLifecycle(
          rows.filter((plan) => !plan.enabled).map((plan) => plan.id),
          true,
        ),
    },
    {
      description: (rows) => {
        const eligible = rows.filter((plan) => plan.enabled);
        return eligible.length > 0
          ? `Review disabling ${eligible.length} selected tunnel plan${eligible.length === 1 ? "" : "s"}.`
          : "All selected tunnel plans are already disabled.";
      },
      disabled: (rows) => pending || rows.every((plan) => !plan.enabled),
    ...
    },
    {
      description: (rows) =>
        rows.length === 1
          ? `Retry the current ${rows[0].enabled ? "apply" : "removal"} state on both endpoints.`
          : "Retry runtime supports one tunnel plan at a time.",
      disabled: (rows) => pending || rows.length !== 1,
      hidden: (rows) =>
        rows.length !== 1 ||
        (!cleanupRetryNeeded(rows[0]) && !applyRetryNeeded(rows[0])),
      ...
      onSelect: (rows) =>
        ...
        requestLifecycle(
          [rows[0].id],
          rows[0].enabled,
          cleanupRetryNeeded(rows[0]),
          applyRetryNeeded(rows[0]),
        ),
    },
    {
      description: (rows) =>
    ...
    <div className="topologyPageStack">
      <section className="fleetPanel tunnelPlanRegistry" ref={listRef}>
        ...
        <ActionFeedback
          ...
          ref={planFeedbackRef}
          tone={error ? "danger" : feedback?.tone}
        />
        <ConsoleDataGrid
          actions={actions}
          columns={columns}
          ...
          empty={
            <div className="emptyState compactEmptyState">
              ...
              <span>
                {loading
                  ? "Reading declarations and endpoint runtime evidence."
                  : "Create an explicit plan before vpsman observes or manages a tunnel."}
              </span>
            </div>
          }
          ...
          rows={tunnelPlans}
          searchPlaceholder="Search plan, endpoint, runtime, connectivity, or OSPF"
          showMobileRowActions={false}
          singleExpandedRow
          storageKey="vpsman.network.tunnelPlans"
          title="Tunnel plans"
          toolbarActions={
      ...
      <ConfirmationPrompt
        ...
        items={
          lifecycleSnapshot
            ? [
                {
                  label: "Action",
                  value: lifecycleSnapshot.retryCleanup
                    ? "Retry runtime removal"
                    : lifecycleSnapshot.retryApply
                      ? "Retry runtime apply"
                      : lifecycleSnapshot.enabled
                        ? "Enable"
                ...
                {
                  label: "Plans",
                  value: lifecycleSnapshot.targets
                    .map(
                      (target) =>
                        `${target.name} (r${target.expected_revision})`,
                    )
                    .join(", "),
                },
                {
                  ...
                  value: lifecycleSnapshot.targets
                    .map((target) => `${target.name}: ${target.declaration}`)
                    .join("; "),
                },
                {
                  label: "Endpoints",
                  value: `${lifecycleSnapshot.targets.length * 2} endpoint configurations`,
                },
        ...
        error={
          lifecycleSnapshot && feedback?.tone === "danger"
            ? feedback.message
            : null
        }
        onCancel={() => setLifecycleSnapshot(null)}
        onConfirm={() =>
          lifecycleSnapshot && void applyLifecycle(lifecycleSnapshot)
        }
        open={lifecycleSnapshot !== null}
        pending={pending}
        title={
          lifecycleSnapshot?.retryCleanup
            ? "Confirm tunnel cleanup retry"
            : lifecycleSnapshot?.retryApply
              ? "Confirm tunnel runtime reapply"
              : lifecycleSnapshot?.enabled
                ? "Confirm tunnel plan enable"
      ...
      <ConfirmationPrompt
        ...
        error={
          deleteSnapshot && feedback?.tone === "danger"
            ? feedback.message
            : null
        }
        items={
          deleteSnapshot
            ? [
                {
                  label: "Plan",
                  value: `${deleteSnapshot.plan.name} (r${deleteSnapshot.target.expected_revision})`,
                },
                {
                  label: "Endpoints",
                  value: `${deleteSnapshot.plan.left_client_id} / ${deleteSnapshot.plan.right_client_id}`,
                },
                {
                  label: "Current state",
                  value: deleteSnapshot.plan.enabled ? "Enabled" : "Disabled",
                },
                {
                  label: "Runtime effect",
                  value: "Queue removal on both endpoints",
                },
                {
                  label: "Current evidence",
                  value: `Left ${tunnelRuntimeConfigLabel(deleteSnapshot.plan.left_runtime_config)}; right ${tunnelRuntimeConfigLabel(deleteSnapshot.plan.right_runtime_config)}`,
                },
...
function TunnelPlanDetails({
  ...
  const [assessmentFeedback, setAssessmentFeedback] = useState<Feedback | null>(
    null,
  );
  useEffect(() => {
    ...
    plan.connection_assessment,
    plan.connection_assessment_note,
    plan.revision,
  ]);
  const assessmentChanged =
    assessment !== (plan.connection_assessment ?? "automatic") ||
  ...
  const assessmentValid =
    ...
    (plan.enabled &&
      assessmentNote.trim().length > 0 &&
      assessmentNote.trim().length <= 500);
  ...
  return (
    <div className="tunnelPlanDetail">
      <div className="tunnelPlanFacts">
        ...
        <PlanFact
          label="Apply state"
          value={`L ${readableTelemetryToken(leftRuntimeState)} · R ${readableTelemetryToken(rightRuntimeState)}`}
        />
        <PlanFact
          label="Connectivity"
          value={`${connectivity.label} · ${connectivity.detail}`}
        />
        <PlanFact
          label="Left outer path"
          value={formatEndpointUnderlay(
            plan.plan.left_local_underlay,
            plan.plan.left_remote_underlay,
          )}
        />
        <PlanFact
          label="Right outer path"
          value={formatEndpointUnderlay(
            plan.plan.right_local_underlay,
            plan.plan.right_remote_underlay,
          )}
        />
        <PlanFact
          label="Tunnel addresses"
          value={formatTunnelAddresses(plan)}
        />
        <PlanFact
          label="Bandwidth"
      ...
      <form
        ...
        onSubmit={(event) => void saveConnectionAssessment(event)}
      >
        <div className="tunnelConnectionAssessmentHeading">
          <strong>Operator connectivity assessment</strong>
          <span title="Use only when endpoint probes cannot establish reachability.">
            Display-only annotation; runtime and automatic OSPF stay
            machine-derived.
          </span>
        </div>
      ...
      {(plan.plan.runtime_control?.manager ?? "agent_iproute2_managed") ===
        ...
        (plan.plan.runtime_topology?.desired_interfaces?.length ?? 0) > 0 && (
          <div className="tunnelIntentLine">
            ...
            <code
              title={plan.plan.runtime_topology?.desired_interfaces?.join(", ")}
            >
              {plan.plan.runtime_topology?.desired_interfaces?.join(", ")}
            </code>
          </div>
        )}
      {plan.plan.conflicts.length > 0 && (
        <ActionFeedback
          message={plan.plan.conflicts.join("; ")}
          tone="warning"
        />
      )}
    </div>
...
function TunnelPlanComposer({
  ...
  const portSpeedClientIds = useMemo(
    () =>
      Array.from(
        new Set([form.leftClientId, form.rightClientId].filter(Boolean)),
      ),
    [form.leftClientId, form.rightClientId],
  );
  const portSpeedSelectionKey = portSpeedClientIds.join("\0");
  const [portSpeedResolution, setPortSpeedResolution] =
    useState<PortSpeedResolution>({
      selectionKey: "",
      speeds: {},
      status: "idle",
    });
  ...
  useEffect(() => {
    ...
    if (!apiToken) {
      setPortSpeedResolution({
        selectionKey: portSpeedSelectionKey,
        speeds: {},
        status: "error",
      });
      return;
    ...
    void Promise.all(
      portSpeedClientIds.map(async (clientId) => {
        const rows = await apiGet<VpsRuleValueRecord[]>(
          `/api/v1/vps-rules?limit=10&client_id=${encodeURIComponent(clientId)}&key=network.port_speed`,
          apiToken,
        );
        const matching = rows.filter(
          (row) =>
            row.client_id === clientId && row.key === "network.port_speed",
        );
        if (matching.length > 1) {
          throw new Error(
            `Multiple port-speed rules were returned for ${clientId}`,
          );
        }
        return [
          clientId,
          matching.length === 0 ? null : portSpeedFromRule(matching[0]),
        ] as const;
      }),
    )
      .then((entries) => {
        ...
        setPortSpeedResolution({
          selectionKey: portSpeedSelectionKey,
          speeds: Object.fromEntries(entries),
          status: "ready",
        });
      })
      .catch(() => {
  ...
  }, [apiToken, initialPlan, portSpeedClientIds, portSpeedSelectionKey]);

  const bandwidthSuggestion = useMemo(
    () =>
      tunnelBandwidthSuggestion(
        portSpeedClientIds,
        portSpeedSelectionKey,
        portSpeedResolution,
      ),
    [portSpeedClientIds, portSpeedResolution, portSpeedSelectionKey],
  );
  useEffect(() => {
    const suggestedValue = bandwidthSuggestion.value;
    if (
      initialPlan ||
      !autoFillOwnership.bandwidth ||
      suggestedValue === null
    ) {
      return;
    }
  ...
  const leftRuntimeDefinitions = definitionsForClient(
    runtimeDefinitions,
    form.leftClientId,
  );
  const rightRuntimeDefinitions = definitionsForClient(
    runtimeDefinitions,
    form.rightClientId,
  );
  const leftRoutingDefinitions = definitionsForClient(
    routingDefinitions,
    form.leftClientId,
  );
  const rightRoutingDefinitions = definitionsForClient(
    routingDefinitions,
    form.rightClientId,
  );
  ...
  function update<K extends keyof TunnelPlanForm>(
    key: K,
    value: TunnelPlanForm[K],
  ) {
    setSnapshot(null);
    setFeedback(null);
  ...
  async function allocate() {
    ...
    try {
      ...
      setFeedback({
        ...
        location: "allocation",
        tone:
          response.ipv4_tunnel || response.ipv6_tunnel ? "success" : "warning",
      });
    } catch (error) {
      setFeedback({
        location: "allocation",
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback({
        location: "form",
        message:
          error instanceof Error ? error.message : "Tunnel plan is invalid",
        tone: "danger",
      });
    }
  }
    ...
    <section className="fleetPanel tunnelPlanComposer">
      <div className="sectionHeader tunnelPlanComposerHeader">
        <div>
          ...
          <span>
            Declare one exact point-to-point tunnel; OSPF remains optional
          </span>
        </div>
        <button
          aria-label="Close tunnel plan editor"
      ...
      <form className="tunnelPlanForm" noValidate onSubmit={submit}>
        <fieldset className="topologyFormSection">
          ...
          <div className="topologyFormGrid threeColumn">
            ...
            <Field
              label="Interface"
              tooltip="Exact interface name to manage or observe on both endpoints."
            >
              <input
                aria-label="Tunnel interface"
                maxLength={15}
                onChange={(event) =>
            ...
            >
              <select
                ...
                onChange={(event) =>
                  update("kind", event.target.value as TunnelKind)
                }
                value={form.kind}
              >
                {kindOptions.map((kind) => (
                  <option key={kind} value={kind}>
                    {formatTunnelKind(kind)}
                  </option>
                ))}
              </select>
            </Field>
          ...
          <div className="topologyFormGrid threeColumn tunnelEndpointUnderlayGrid">
            <Field label="Left VPS">
              <VpsCombobox
                agents={agents}
                ariaLabel="Left tunnel VPS"
                excludeIds={form.rightClientId ? [form.rightClientId] : []}
                onChange={(value) => update("leftClientId", value)}
                value={form.leftClientId}
              />
            </Field>
            <Field
            ...
            >
              <input
                ...
                onChange={(event) => {
                  ...
                  update("leftRemoteUnderlay", event.target.value);
                }}
                placeholder="Peer destination IP"
                required
            ...
            <Field label="Right VPS">
              <VpsCombobox
                agents={agents}
                ariaLabel="Right tunnel VPS"
                excludeIds={form.leftClientId ? [form.leftClientId] : []}
                onChange={(value) => update("rightClientId", value)}
                value={form.rightClientId}
              />
            </Field>
            <Field
            ...
            >
              <input
                ...
                onChange={(event) => {
                  ...
                  update("rightRemoteUnderlay", event.target.value);
                }}
                placeholder="Peer destination IP"
                required
        ...
        <fieldset className="topologyFormSection">
          ...
          <div
            className="runtimeManagerSegments"
            role="radiogroup"
          ...
          >
            ...
            <ManagerChoice
              ...
              detail="vpsman inspects this exact declared interface and never mutates it."
          ...
          {form.runtimeManager === "external_managed_adapter" && (
            <div className="topologyFormGrid twoColumn adapterBindingGrid">
              <AdapterDefinitionField
                ...
                domain="runtime_tunnel"
                label="Left runtime adapter"
                onChange={(value) => update("leftRuntimeDefinitionId", value)}
                onOpenAdapterDefinitions={onOpenAdapterDefinitions}
                definitions={leftRuntimeDefinitions}
                value={form.leftRuntimeDefinitionId}
              />
              <AdapterDefinitionField
                clientId={form.rightClientId}
                domain="runtime_tunnel"
                label="Right runtime adapter"
                onChange={(value) => update("rightRuntimeDefinitionId", value)}
                onOpenAdapterDefinitions={onOpenAdapterDefinitions}
                definitions={rightRuntimeDefinitions}
                value={form.rightRuntimeDefinitionId}
              />
            </div>
          )}
          {form.runtimeManager !== "external_observed" && (
            <div className="topologyFormGrid fourColumn compactNumericGrid">
              ...
              >
                <UnitInput
                  ariaLabel="Ingress limit"
                  max={1_000}
                  min={0.064}
                  onChange={(value) => update("ingressMbps", value)}
                  step="0.001"
                  unit="Mbps"
                  value={form.ingressMbps}
                />
              </Field>
              <Field
                label="Egress limit"
                tooltip="Optional egress shaping limit from 0.064 to 1000 Mbps. This remains separate from display-only port speed."
              >
                <UnitInput
                  ariaLabel="Egress limit"
                  max={1_000}
                  min={0.064}
                  onChange={(value) => update("egressMbps", value)}
                  step="0.001"
                  unit="Mbps"
                  value={form.egressMbps}
                />
              </Field>
              <Field
                label="Burst"
                tooltip="Optional traffic shaping burst from 1 to 1048576 KiB."
              >
                <UnitInput
                  ariaLabel="Traffic burst"
                  max={1_048_576}
                  min={1}
                  onChange={(value) => update("burstKb", value)}
                  unit="KiB"
                  value={form.burstKb}
                />
              </Field>
          ...
          {form.kind === "fou" &&
            form.runtimeManager !== "external_observed" && (
              <div className="topologyFormGrid threeColumn compactNumericGrid">
                <Field
                  label="FOU port"
                  tooltip="UDP port registered locally by ip fou on each endpoint."
                >
                  <UnitInput
                    ariaLabel="FOU local port"
                    max={65535}
                    min={1}
                    onChange={(value) => update("fouPort", value)}
                    unit="port"
                    value={form.fouPort}
                  />
                </Field>
                <Field
                  label="Peer port"
                  tooltip="Remote UDP destination port used for FOU encapsulation."
                >
                  <UnitInput
                    ariaLabel="FOU peer port"
                    max={65535}
                    min={1}
                    onChange={(value) => update("fouPeerPort", value)}
                    unit="port"
                    value={form.fouPeerPort}
                  />
                </Field>
                <Field
                  label="IP protocol"
              ...
              <div className="topologyFormGrid twoColumn">
                ...
                >
                  <textarea
                    ...
                    placeholder="Optional additional exact interfaces"
                    rows={3}
                    value={form.desiredInterfaces}
                  />
                </Field>
                <Field
                ...
                >
                  <textarea
                    ...
                    placeholder="One exact interface per line"
                    rows={3}
                    value={form.staleInterfaces}
                  />
                </Field>
                <Field
                ...
                >
                  <textarea
                    ...
                    placeholder="10.0.0.0/24, dev=tun0"
                    rows={3}
                    value={form.staleRoutes}
                  />
                </Field>
              </div>
            </details>
        ...
        <fieldset className="topologyFormSection">
          ...
          <div className="addressAllocationToolbar">
            <label className="compactCheckbox">
              <input
                checked={form.includeIpv4}
                onChange={(event) =>
                  update("includeIpv4", event.target.checked)
                }
                type="checkbox"
              />{" "}
              IPv4
            ...
            <label className="compactCheckbox">
              <input
                ...
                onChange={(event) =>
                  update("includeIpv6", event.target.checked)
                }
                type="checkbox"
              />{" "}
              IPv6
            ...
            {form.includeIpv4 && form.includeIpv6 && (
              <label
                className="addressFamilySelect"
                title="Primary address family for continuous latency evidence. The other family remains configured."
              >
                <span>Probe family</span>
                <select
                  aria-label="Latency probe address family"
                  onChange={(event) =>
                    update(
                      "latencyPrimaryFamily",
                      event.target.value as "ipv4" | "ipv6",
                    )
                  }
                  value={form.latencyPrimaryFamily}
                >
            ...
            <button
              className="secondaryAction compactAction"
              disabled={
                allocationPending || (!form.includeIpv4 && !form.includeIpv6)
              }
            ...
            >
              <Settings2 size={14} />
              {allocationPending ? "Allocating" : "Allocate"}
            </button>
          </div>
            ...
            <div className="topologyFormGrid fourColumn compactNumericGrid">
              ...
              >
                <input
                  ...
                  onChange={(event) => update("ipv4Pool", event.target.value)}
                  placeholder="Configured pool"
                  value={form.ipv4Pool}
                />
              </Field>
              <Field label="Left IPv4">
                <input
                  aria-label="Left tunnel IPv4"
                  onChange={(event) => update("leftIpv4", event.target.value)}
                  placeholder="10.255.0.0"
                  value={form.leftIpv4}
                />
              </Field>
              <Field label="Right IPv4">
                <input
                  aria-label="Right tunnel IPv4"
                  onChange={(event) => update("rightIpv4", event.target.value)}
                  placeholder="10.255.0.1"
                  value={form.rightIpv4}
                />
              </Field>
              <Field label="Prefix">
                <UnitInput
                  ariaLabel="IPv4 tunnel prefix"
                  max={32}
                  min={0}
                  onChange={(value) => update("ipv4Prefix", value)}
                  unit="bits"
            ...
            <div className="topologyFormGrid fourColumn compactNumericGrid">
              ...
              >
                <input
                  ...
                  onChange={(event) => update("ipv6Pool", event.target.value)}
                  placeholder="Configured pool"
                  value={form.ipv6Pool}
                />
              </Field>
              <Field label="Left IPv6">
                <input
                  aria-label="Left tunnel IPv6"
                  onChange={(event) => update("leftIpv6", event.target.value)}
                  placeholder="fd00::"
                  value={form.leftIpv6}
                />
              </Field>
              <Field label="Right IPv6">
                <input
                  aria-label="Right tunnel IPv6"
                  onChange={(event) => update("rightIpv6", event.target.value)}
                  placeholder="fd00::1"
                  value={form.rightIpv6}
                />
              </Field>
              <Field label="Prefix">
                <UnitInput
                  ariaLabel="IPv6 tunnel prefix"
                  max={128}
                  min={0}
                  onChange={(value) => update("ipv6Prefix", value)}
                  unit="bits"
        ...
        <fieldset className="topologyFormSection ospfOptionalSection">
          ...
          <label
            ...
            title="Optional external routing-cost control. Tunnel creation and observation do not require OSPF."
          >
            <input
              checked={form.ospfEnabled}
              onChange={(event) => update("ospfEnabled", event.target.checked)}
              type="checkbox"
            />
            <span>
              <strong>Enable OSPF cost control</strong>
              <small>
                Each endpoint uses its Configuration preset unless this plan
                selects an override.
              </small>
            </span>
          </label>
            ...
            <>
              <div className="topologyFormGrid twoColumn adapterBindingGrid">
                <AdapterDefinitionField
                  clientId={form.leftClientId}
                  domain="routing_cost"
                  emptyLabel={endpointOspfPresetOptionLabel(
                    leftOspfSource,
                    configurationSourcesEvidenceState,
                  )}
                  label="Left OSPF command override (optional)"
                  onChange={(value) => update("leftRoutingDefinitionId", value)}
                  onOpenAdapterDefinitions={onOpenAdapterDefinitions}
                  definitions={leftRoutingDefinitions}
                  value={form.leftRoutingDefinitionId}
                />
                <AdapterDefinitionField
                  clientId={form.rightClientId}
                  domain="routing_cost"
                  ...
                  label="Right OSPF command override (optional)"
                  onChange={(value) =>
                    update("rightRoutingDefinitionId", value)
                  }
                  onOpenAdapterDefinitions={onOpenAdapterDefinitions}
                  definitions={rightRoutingDefinitions}
              ...
              <span className="formHint">
                A per-plan override takes precedence over that VPS&apos;s
                effective preset.{" "}
                <button
              ...
              <div className="topologyFormGrid ospfControlGrid">
                <Field
                  label="Control mode"
                  tooltip="Reviewed waits for operator confirmation. Automatic is executed only by the server controller after configured health gates."
                >
                  <select
                    aria-label="OSPF control mode"
                    onChange={(event) =>
                      update(
                        "ospfMode",
                        event.target.value as "reviewed" | "automatic",
                      )
                    }
                    value={form.ospfMode}
                  >
                ...
                <Field
                  ...
                  tooltip="Baseline round-trip latency used only until recent probe evidence exists."
                >
                  <UnitInput
                    ariaLabel="Planned tunnel latency"
                    min={0}
                    onChange={(value) => update("plannedLatencyMs", value)}
                    required
                    step="0.1"
                    unit="ms"
                    value={form.plannedLatencyMs}
                  />
                </Field>
                <Field
                  label="Planned loss"
                  tooltip="Baseline packet loss used for the planned cost preview; measured probes replace it when available."
                >
                  <UnitInput
                    ariaLabel="Planned packet loss"
                    max={100}
                    min={0}
                ...
                <Field
                  label="Preference"
                  tooltip="Values above 1 make the tunnel more preferred by lowering cost; values below 1 raise cost."
                >
                  <input
                    aria-label="Tunnel preference"
                    max={100}
                    min={0.1}
                    onChange={(event) =>
                      update("preference", event.target.value)
                    }
                    step="0.1"
                    type="number"
        ...
        <div className="tunnelComposerFooter">
          <label className="toggleLine compactToggle">
            ...
            <span>
              <strong>
                {existing ? "Enabled after update" : "Enable after save"}
              </strong>
              <small>Push runtime state to both endpoints immediately.</small>
            </span>
          </label>
            ...
            <button
              ...
              title={
                pending
                  ? "Saving tunnel plan"
                  : unchanged
                    ? "No tunnel plan changes to review"
                    : (validationError ??
                      "Review the exact declaration before saving")
              }
              type="submit"
            >
              <CirclePlus size={16} />
              {existing ? "Review update" : "Review plan"}
            </button>
          </div>
        </div>
      </form>
      <ConfirmationPrompt
        ...
        title={
          existing
            ? "Confirm tunnel plan update"
            : "Confirm tunnel plan creation"
        }
        tone="normal"
      />
    </section>
...
function AdapterDefinitionField({
  clientId,
  domain,
  emptyLabel = "Select adapter definition",
  label,
  onChange,
  onOpenAdapterDefinitions,
  definitions,
  ...
  return (
    ...
    >
      ...
      <div className="adapterDefinitionSelectRow">
          ...
          <option value="">
            {!clientId
              ...
              : domain !== "routing_cost" && definitions.length === 0
                ? "No compatible adapter definitions"
                : emptyLabel}
          </option>
        ...
        <button
          aria-label={`Open adapter definitions for ${label}`}
          className="iconButton"
          onClick={() => onOpenAdapterDefinitions(domain)}
          title={`Create or manage ${domain === "routing_cost" ? "routing cost" : "tunnel runtime"} adapters`}
          type="button"
        >
...
function tunnelRetirementFeedback(
  ...
  const feedback = tunnelDispatchFeedback(
    sync,
    savedMessage,
    "Runtime removal",
  );
...
function tunnelConnectivityPresentation(
  ...
  if (plan.connection_assessment !== "automatic") {
    const label =
      plan.connection_assessment === "connected" ? "Connected" : "Disconnected";
...
function tunnelConnectionAssessmentError(error: unknown): string {
  ...
  return error instanceof Error
    ? error.message
    : "Connectivity assessment update failed";
}
...
function tunnelPlanFormFromRecord(record: TunnelPlanRecord): TunnelPlanForm {
  ...
  return {
    ...
    ingressMbps: kbpsToMbpsText(traffic.ingress_kbps),
...
function formatRuntimeRoute(route: RuntimeTunnelRoute): string {
  return [
    ...
    route.metric !== null && route.metric !== undefined
      ? `metric=${route.metric}`
      : null,
  ]
    .filter(Boolean)
...
function kbpsToMbpsText(value: number | null | undefined): string {
  return value === null || value === undefined ? "" : String(value / 1_000);
}

function mbpsToKbpsText(value: string): string {
...
function validateTunnelPlanForm(form: TunnelPlanForm): string | null {
  ...
  if (!form.leftClientId || !form.rightClientId)
    return "Select both endpoint VPSs";
  if (form.leftClientId === form.rightClientId)
    return "Tunnel endpoints must be different VPSs";
  ...
  if (
    ...
    bandwidth < MIN_TUNNEL_BANDWIDTH_MBPS ||
  ...
  if (form.runtimeManager !== "external_observed") {
    const trafficError =
      ...
      validateOptionalMbpsLimit(form.egressMbps, "Egress limit") ??
  ...
  if (form.includeIpv4) {
    if (!form.leftIpv4.trim() || !form.rightIpv4.trim())
      return "Enter or allocate both IPv4 endpoint addresses";
    if (
      !isIpv4Address(form.leftIpv4) ||
      !isIpv4Address(form.rightIpv4) ||
      form.leftIpv4.trim() === form.rightIpv4.trim()
    )
      return "IPv4 endpoint addresses must be valid and different";
    const prefixError = validateIntegerRange(
      form.ipv4Prefix,
      "IPv4 prefix",
      0,
      32,
    ...
    if (
      !sameIpSubnet(form.leftIpv4, form.rightIpv4, Number(form.ipv4Prefix), 32)
    )
      return "IPv4 endpoint addresses must share the selected point-to-point prefix";
    if (form.ipv4Pool.trim() && !isIpCidr(form.ipv4Pool, 32))
      return "IPv4 allocation pool must be a valid CIDR";
  }
  if (form.includeIpv6) {
    if (!form.leftIpv6.trim() || !form.rightIpv6.trim())
      return "Enter or allocate both IPv6 endpoint addresses";
    if (
      !isIpv6Address(form.leftIpv6) ||
      !isIpv6Address(form.rightIpv6) ||
      form.leftIpv6.trim() === form.rightIpv6.trim()
    )
      return "IPv6 endpoint addresses must be valid and different";
    const prefixError = validateIntegerRange(
      form.ipv6Prefix,
      "IPv6 prefix",
      0,
    ...
    if (prefixError) return prefixError;
    if (
      !sameIpSubnet(form.leftIpv6, form.rightIpv6, Number(form.ipv6Prefix), 128)
    )
      return "IPv6 endpoint addresses must share the selected point-to-point prefix";
    if (form.ipv6Pool.trim() && !isIpCidr(form.ipv6Pool, 128))
  ...
  if (form.ospfEnabled) {
    const ospfError =
      validateNumberRange(
        form.plannedLatencyMs,
        "Planned latency",
        0,
        60_000,
      ) ??
      validateNumberRange(
        form.packetLossPercent,
        "Planned packet loss",
        0,
        100,
      ) ??
  ...
  if (
    form.ospfEnabled &&
    (!Number.isInteger(minCost) ||
      !Number.isInteger(maxCost) ||
      minCost < 1 ||
      minCost > maxCost ||
      maxCost > 65535)
  )
    return "OSPF cost bounds must be whole numbers within 1-65535 and minimum cannot exceed maximum";
  return null;
}
...
function validateOptionalMbpsLimit(
  value: string,
  label: string,
): string | null {
  ...
  if (!/^(?:0|[1-9]\d*)(?:\.\d{1,3})?$/.test(trimmed)) {
    return `${label} must use Mbps with at most three decimal places`;
  }
  const parsed = Number(trimmed);
  const kbps = parsed * 1_000;
  if (
    !Number.isFinite(parsed) ||
    parsed < 0.064 ||
    parsed > 1_000 ||
    Math.abs(kbps - Math.round(kbps)) > 0.000_001
  ) {
    return `${label} must be from 0.064 to 1000 Mbps in 0.001 Mbps steps`;
  }
  return null;
}
...
function sameIpSubnet(
  ...
  if (leftValue === null || rightValue === null || prefix < 0 || prefix > bits)
    return false;
...
function validateExistingTunnelPlanConflicts(
  ...
  for (const plan of plans) {
    ...
    if (
      ...
      (clientIds.has(plan.left_client_id) ||
        clientIds.has(plan.right_client_id))
    ) {
...
function validateAdapterBindings(
  ...
  if (form.runtimeManager === "external_managed_adapter") {
    ...
    const right = rightRuntimeDefinitions.find(
      (definition) => definition.id === form.rightRuntimeDefinitionId,
    );
    if (form.leftRuntimeDefinitionId && !left)
      return "The left runtime adapter is no longer available for this VPS";
    if (form.rightRuntimeDefinitionId && !right)
      return "The right runtime adapter is no longer available for this VPS";
    ...
    if (
      hasTrafficLimit &&
      (!adapterHasDefinitionField(left, "traffic_limit_command") ||
        !adapterHasDefinitionField(right, "traffic_limit_command"))
    ) {
  ...
  if (form.ospfEnabled) {
    if (
      form.leftRoutingDefinitionId &&
      !leftRoutingDefinitions.some(
        (definition) => definition.id === form.leftRoutingDefinitionId,
      )
    ) {
    ...
    if (
      form.rightRoutingDefinitionId &&
      !rightRoutingDefinitions.some(
        (definition) => definition.id === form.rightRoutingDefinitionId,
      )
    ) {
...
function buildTunnelPlanRequest(form: TunnelPlanForm): CreateTunnelPlanRequest {
  ...
  return {
    ...
    ipv4_tunnel: form.includeIpv4
      ? addressPair(form.leftIpv4, form.rightIpv4, form.ipv4Prefix)
      : null,
    ipv6_address_pool_cidr: form.ipv6Pool.trim() || null,
    ipv6_tunnel: form.includeIpv6
      ? addressPair(form.leftIpv6, form.rightIpv6, form.ipv6Prefix)
      : null,
    kind: form.kind,
...
function applyAllocation(
  form: TunnelPlanForm,
  response: AllocateTunnelEndpointsResponse,
): TunnelPlanForm {
  return {
    ...form,
...
function createConfirmationItems(
  ...
  const adapterDefinitionName = (definitionId: string | null | undefined) =>
    networkAdapterDefinitions.find(
      (definition) => definition.id === definitionId,
    )?.name ?? (definitionId ? shortId(definitionId) : "not bound");
  const trafficSummary =
    [
      traffic?.ingress_kbps
        ? `ingress ${kbpsToMbpsText(traffic.ingress_kbps)} Mbps`
        : null,
      traffic?.egress_kbps
        ? `egress ${kbpsToMbpsText(traffic.egress_kbps)} Mbps`
        : null,
  ...
  return [
    {
      label: "Operation",
      value: existing
        ? `Update declaration ${existing.id} at revision ${existing.revision}`
        : "Create declaration",
    },
    {
      label: "Plan",
      value: `${request.name} · ${formatTunnelKind(request.kind)} · ${request.interface_name}`,
    },
    {
      label: "Endpoints",
      value: `${request.left_client_id} / ${request.right_client_id}`,
    },
    {
      label: "Left outer path",
      value: formatEndpointUnderlay(
        request.left_local_underlay,
        request.left_remote_underlay,
      ),
    },
    {
      label: "Right outer path",
      value: formatEndpointUnderlay(
        request.right_local_underlay,
        request.right_remote_underlay,
      ),
    },
    {
      label: "Runtime owner",
      ...
      : []),
    {
      ...
      value: [
        request.ipv4_tunnel
          ...
          : null,
        request.ipv6_tunnel
          ? `${request.ipv6_tunnel.left}/${request.ipv6_tunnel.prefix_len} / ${request.ipv6_tunnel.right}/${request.ipv6_tunnel.prefix_len}`
          : null,
      ]
        .filter(Boolean)
        .join("; "),
      ...
      : []),
    {
      ...
      value: request.enabled
        ? "Enable and push to both endpoints"
        : "Save disabled",
    },
  ];
}
...
function tunnelPlanFormMatchesInitial(
  ...
  try {
    return (
      JSON.stringify(buildTunnelPlanRequest(form)) ===
      JSON.stringify(
        buildTunnelPlanRequest(tunnelPlanFormFromRecord(initialPlan)),
      )
    );
  } catch {
    return false;
...
type PortSpeedResolution = {
  selectionKey: string;
  speeds: Record<string, PortSpeedRule | null>;
  status: "idle" | "loading" | "ready" | "error";
};

type TunnelBandwidthSuggestion = {
  detail: string | null;
  value: string | null;
};

function portSpeedFromRule(rule: VpsRuleValueRecord): PortSpeedRule {
  if (rule.state !== "ok" || rule.validation_errors.length > 0) {
    throw new Error(`Port-speed rule for ${rule.client_id} is invalid`);
  }
  const value = rule.value_json;
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error(
      `Port-speed rule for ${rule.client_id} has no structured value`,
    );
  }
  const fields = value as Record<string, unknown>;
  const bps = fields.bps;
  ...
  if (
    ...
    throw new Error(`Port-speed rule for ${rule.client_id} is malformed`);
  }
  return { bps: Number(bps), display: display.trim() };
}

function tunnelBandwidthSuggestion(
  clientIds: string[],
  selectionKey: string,
  resolution: PortSpeedResolution,
): TunnelBandwidthSuggestion {
  if (clientIds.length === 0) {
    return {
      detail: null,
      value: String(DEFAULT_TUNNEL_BANDWIDTH_MBPS),
    };
  }
  if (
    resolution.selectionKey !== selectionKey ||
    resolution.status === "idle"
  ) {
    return { detail: null, value: null };
  }
  ...
  const configured = clientIds
    .map((clientId) => resolution.speeds[clientId] ?? null)
    .filter((speed): speed is PortSpeedRule => speed !== null);
  ...
  const minimum = configured.reduce((current, speed) =>
    speed.bps < current.bps ? speed : current,
  );
  const exactMbps = minimum.bps / 1_000_000;
  if (
    !Number.isInteger(exactMbps) ||
    exactMbps < MIN_TUNNEL_BANDWIDTH_MBPS ||
    exactMbps > MAX_TUNNEL_BANDWIDTH_MBPS
  ) {
    return {
      detail: null,
      value: "",
  ...
  const source =
    configured.length === 1
      ? `${minimum.display} from the configured endpoint`
      : `${configured.map((speed) => speed.display).join(" and ")}; lower value ${minimum.display}`;
  return {
    detail: `Planning bandwidth suggested from ${source}. This is editable and does not configure traffic shaping.`,
    value: String(exactMbps),
  };
}

function observedPeerAddress(
  agents: AgentView[],
  clientId: string,
  manager: RuntimeTunnelManager,
): string | null {
...
type TopologyPanelProps = {
  ...
  onLoadPortForwardRules: () => Promise<string | null>;
  onMutatePortForwardRule: (
    ruleId: string,
