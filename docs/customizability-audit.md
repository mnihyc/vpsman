...
- Operators should manage built-in default presets, shared customizable
  presets, and VPS-local custom presets through preset management. Pool/tag and
  bulk workflows are convenience tools for assigning, cloning, testing, or
  updating those preset objects at scale; they are not ad hoc command updates.
...
- Backup, restore, and update:
  - Current state: local filesystem object store with reserved S3 extension;
    ...
    failed rollout counts, and delegated rollout-proof readiness.
...
- Frontend frequent-use configuration:
  - Current state: the Pools view now has a data-source preset manager backed
    by server storage. Operators can save shared presets, save VPS-local
    presets, see built-in/default presets and assignment counts, and assign the
    selected preset by VPS, pool, or tag with API confirmation semantics. The
    panel, API, CLI, and VTY can clone, diff, test, update, and preview the
    read-only hot-config fragment rendered from a VPS's selected presets. The
    first Active source status read model is visible in API, CLI/VTY, and the
    Pools panel, including backup/update object-store readiness evidence,
    proof-gated on-demand workflow evidence, and process-limit capability
    readiness for root-capable, unknown, and unprivileged agents. The Pools
    panel now uses shared CRUD/list controls for active source rows and preset
    registry rows, including total/filtered counts, current page, field search,
    and page controls; the same abstraction is used for audit, job, and
    schedule record tables.
  - Required remaining model: active-preset badges in each operational module,
    ...
    workflows, richer curated provider libraries, and proof-gated dispatch of
    rendered fragments.
...
- Data-source bulk update must remain preset-centric: updating a shared preset
  ...
  Pool/tag/client assignment only changes which VPSs select a preset.
...
- This is a typed adapter-field conversion, not a data-source bulk command
  ...
  the affected plans before proof-gated mutation.
- Product-code hotspot: telemetry promotion defaults in
  ...
  named operator presets/profiles with explicit VPS assignment or pool/tag
  assignment helpers rather than compiled business policy.
...
- Remaining open candidates are not release-complete. The current list is
  mostly installer/autostart policy paths, shell/test fixtures, proof/backup
