...
smoke_enter_root
smoke_require_tools curl docker jq shuf timeout

if [[ "${VPSMAN_SMOKE_SKIP_BUILD:-0}" != "1" ]]; then
  cargo build -p vpsman-api -p vpsctl -p vpsman-worker
fi
...
vpsctl_json() {
  ...
  VPSMAN_SUPER_PASSWORD="postgres-smoke-super-password" \
  VPSMAN_SUPER_SALT_HEX="01020304" \
    target/debug/vpsctl "$@"
}
...
api_post "/api/v1/agents/pg-agent-b/tags" '{"tag":"edge"}' >/dev/null
api_post "/api/v1/agents/pg-agent-b/tags" '{"tag":"bird2"}' >/dev/null
...
' >/dev/null
telemetry_bucket=$(( (($(date +%s) - 600) / 300) * 300 ))
seed_telemetry "pg-agent-a" "$((telemetry_bucket + 60))" 0.5 134217728 5368709120 1000 2000
seed_telemetry "pg-agent-a" "$((telemetry_bucket + 120))" 1.0 100663296 4294967296 4000 8000
...
VPSMAN_MIGRATIONS_DIR="$ROOT_DIR/migrations" \
  target/debug/vpsman-worker --once --worker-id pg-postgres-smoke --telemetry-rollup-secs 300 --telemetry-rollup-lookback-hours 1 --telemetry-prune-after-hours 1 >/dev/null
vpsctl_json telemetry-rollups --client-id pg-agent-a --bucket-secs 300 --limit 10 | jq -e '
  any(.[]; .client_id == "pg-agent-a" and .bucket_secs == 300 and .sample_count == 2 and
...
' >/dev/null
vpsctl_json telemetry-network-rates --client-id pg-agent-a --interface eth0 --bucket-secs 300 --limit 10 | jq -e '
  any(.[]; .client_id == "pg-agent-a" and .interface == "eth0" and .bucket_secs == 300 and
    .sample_count == 2 and
    .rx_bytes_delta == 3000 and
    .tx_bytes_delta == 6000 and
    (.rx_bps_avg > 399 and .rx_bps_avg < 401) and
    (.tx_bps_avg > 799 and .tx_bps_avg < 801))
' >/dev/null
old_telemetry_count="$(docker exec "$container_name" psql -U vpsman -d vpsman -tAc "SELECT count(*) FROM telemetry_samples WHERE observed_at < now() - interval '1 hour'")"
if [[ "$old_telemetry_count" != "0" ]]; then
  echo "expected worker telemetry pruning to remove old samples, found $old_telemetry_count" >&2
  exit 1
fi

pool_json="$(api_post "/api/v1/pools" '{"name":"pg-pool","provider":"postgres-provider","region":"pg-region"}')"
pool_id="$(jq -r '.id' <<<"$pool_json")"
api_post "/api/v1/agents/pg-agent-a/pool" "{\"pool_id\":\"$pool_id\"}" >/dev/null
api_post "/api/v1/agents/pg-agent-a/tags" '{"tag":"persistent"}' >/dev/null
...
' >/dev/null
api_get "/api/v1/pools" | jq -e --arg pool_id "$pool_id" '
  any(.[]; .id == $pool_id and (.clients | length == 1) and .clients[0].id == "pg-agent-a")
' >/dev/null
...
vpsctl_json key-lifecycle-report | jq -e '
  .current_key_revoked_count == 1 and
  .revocation_count >= 1 and
  any(.clients[]; .client_id == "pg-agent-a" and .current_key_revoked == true)
' >/dev/null
...
schedule_json="$(api_post "/api/v1/schedules" '{
...
jq -e '.name == "pg-hourly-uptime" and .enabled == true and .command_type == "shell_argv" and (.tags == ["edge"]) and .catch_up_policy == "run_all_limited" and .catch_up_limit == 2 and .retry_delay_secs == 120 and .max_failures == 5 and .failure_count == 0' \
...
VPSMAN_POSTGRES_URL="$postgres_url" \
VPSMAN_MIGRATIONS_DIR="$ROOT_DIR/migrations" \
  target/debug/vpsman-worker --once --worker-id pg-postgres-smoke --telemetry-rollup-secs 300 --telemetry-rollup-lookback-hours 1 --telemetry-prune-after-hours 1 --notification-retention-days 30 >"$SMOKE_TMPDIR/worker-once.log" 2>&1
approval_jobs_json="$(api_get "/api/v1/jobs?limit=20" | jq '
  map(select((.command_type | startswith("scheduled_")) and .status == "approval_required"))
')"
approval_job_id="$(jq -r 'first | .id // empty' <<<"$approval_jobs_json")"
if [[ -z "$approval_job_id" ]]; then
  echo "scheduled approval job was not materialized" >&2
...
if [[ "$(jq 'length' <<<"$approval_jobs_json")" -ne "2" ]]; then
  echo "expected run_all_limited schedule catch-up to materialize two approval jobs" >&2
  exit 1
fi
...
' >/dev/null
worker_lease_count="$(docker exec "$container_name" psql -U vpsman -d vpsman -tAc "SELECT count(*) FROM worker_leases WHERE task_name IN ('schedules','alert_notifications','telemetry_rollups','telemetry_network_rates','telemetry_prune') AND owner = 'pg-postgres-smoke' AND lease_expires_at > now()")"
if [[ "$worker_lease_count" != "5" ]]; then
  echo "expected active worker lease rows for all singleton tasks" >&2
...
cancel_json="$(vpsctl_json job-cancel \
  --job-id "$approval_job_id" \
  --reason "postgres persistence cancellation" \
  --confirmed)"
jq -e --arg job_id "$approval_job_id" '.job_id == $job_id and .canceled == true and .status == "canceled" and .canceled_targets == 2' \
  <<<"$cancel_json" >/dev/null
api_get "/api/v1/jobs/$approval_job_id" | jq -e --arg job_id "$approval_job_id" '
  .id == $job_id and (.command_type | startswith("scheduled_")) and .status == "canceled" and .completed_at != null
' >/dev/null
api_get "/api/v1/jobs/$approval_job_id/targets" | jq -e '
  length == 2 and
  (map(.client_id) | sort == ["pg-agent-a","pg-agent-b"]) and
  all(.[]; .status == "canceled" and .completed_at != null)
' >/dev/null
...
VPSMAN_POSTGRES_URL="$postgres_url" \
VPSMAN_MIGRATIONS_DIR="$ROOT_DIR/migrations" \
  target/debug/vpsman-worker --once --worker-id pg-competing-worker --telemetry-rollup-secs 300 --telemetry-rollup-lookback-hours 1 --telemetry-prune-after-hours 1 --notification-retention-days 30 >/dev/null
...
api_post_expect_status "/api/v1/jobs/$approval_job_id/dispatch-scheduled" '{"confirmed":true,"timeout_secs":5,"envelope":null,"envelopes":{}}' "404" >/dev/null
...
jq -e '.client_id == "pg-agent-a" and .status == "requested_metadata_only" and .include_config == true and .proof_scope == "client:pg-agent-a" and .artifact_id == null' \
...
jq -e --arg backup_id "$backup_id" '.source_backup_request_id == $backup_id and .source_client_id == "pg-agent-a" and .target_client_id == "pg-agent-b" and .status == "planned_metadata_only" and .destination_root == "/restore" and .proof_scope == "client:pg-agent-b"' \
...
rejected_job_id="$(jq -r '.job_id' <<<"$rejected_job_json")"
jq -e '.accepted_targets == 0 and .status == "rejected_authorization_required"' \
  <<<"$rejected_job_json" >/dev/null
api_get "/api/v1/jobs/$rejected_job_id" | jq -e --arg job_id "$rejected_job_id" '
  .id == $job_id and .command_type == "api_job_request" and .status == "rejected_authorization_required" and .target_count == 2
' >/dev/null
api_get "/api/v1/jobs/$rejected_job_id/targets" | jq -e '
  length == 2 and
  (map(.client_id) | sort == ["pg-agent-a","pg-agent-b"]) and
  all(.[]; .status == "rejected_authorization_required" and .completed_at != null)
' >/dev/null

command_template_request="$(jq -n '{
  name: "pg-smoke-pool-uptime",
  scope_kind: "pool",
  scope_value: "pg-smoke-pool",
  command_type: "shell",
  operation: {
    type: "shell",
    argv: ["/usr/bin/uptime"],
...
jq -e '.name == "pg-smoke-pool-uptime" and .scope_kind == "pool" and .scope_value == "pg-smoke-pool" and .operation.type == "shell"' \
  <<<"$command_template_json" >/dev/null
api_get "/api/v1/command-templates?limit=20&scope_kind=pool&scope_value=pg-smoke-pool" | jq -e --arg template_id "$command_template_id" '
  any(.[]; .id == $template_id and .name == "pg-smoke-pool-uptime" and .command_type == "shell")
' >/dev/null
...
idempotent_reject_job_id="$(jq -r '.job_id' <<<"$idempotent_reject_first")"
idempotent_reject_second="$(api_post_expect_status "/api/v1/jobs" "$idempotent_reject_payload" "200")"
jq -e --arg job_id "$idempotent_reject_job_id" '
  .job_id == $job_id and .accepted_targets == 0 and .status == "rejected_authorization_required"
' <<<"$idempotent_reject_second" >/dev/null
idempotent_conflict_payload="$(jq -n '{
  targets: [],
  clients: ["pg-agent-a"],
  pools: [],
  tags: [],
...
api_post_expect_status "/api/v1/jobs" "$idempotent_conflict_payload" "409" >/dev/null

api_get "/api/v1/audit" | jq -e '
  ...
  any(.[]; .action == "schedule.upserted") and
  ...
  any(.[]; .action == "job.canceled") and
  any(.[]; .action == "command_template.upserted") and
  any(.[]; .action == "job.rejected_authorization_required")
' >/dev/null
...
api_get "/api/v1/fleet/summary" | jq -e '.total == 2 and .connected == 2' >/dev/null
api_get "/api/v1/agents" | jq -e '
...
api_get "/api/v1/key-lifecycle/report" | jq -e '
  ...
  any(.clients[]; .client_id == "pg-agent-a" and .current_key_revoked == false)
' >/dev/null
api_get "/api/v1/telemetry/rollups?client_id=pg-agent-a&bucket_secs=300&limit=10" | jq -e '
  any(.[]; .client_id == "pg-agent-a" and .sample_count == 2 and
    ...
    .network_tx_bytes_max == 8000)
' >/dev/null
api_get "/api/v1/telemetry/network-rates?client_id=pg-agent-a&interface=eth0&bucket_secs=300&limit=10" | jq -e '
...
' >/dev/null
api_get "/api/v1/pools" | jq -e --arg pool_id "$pool_id" '
  any(.[]; .id == $pool_id and .name == "pg-pool" and (.clients | length == 1) and .clients[0].id == "pg-agent-a")
' >/dev/null
api_post "/api/v1/bulk/resolve" "{\"clients\":[],\"pools\":[\"$pool_id\"],\"tags\":[\"edge\"],\"destructive\":false,\"confirmed\":false}" \
...
' >/dev/null
api_get "/api/v1/schedules" | jq -e --arg schedule_id "$schedule_id" '
  any(.[]; .id == $schedule_id and .name == "pg-hourly-uptime" and .enabled == true and .command_type == "shell_argv" and (.tags == ["edge"]))
' >/dev/null
api_get "/api/v1/backups?limit=10" | jq -e --arg backup_id "$backup_id" --arg artifact_id "$artifact_id" '
  any(.[]; .id == $backup_id and .client_id == "pg-agent-a" and .status == "artifact_metadata_recorded" and .include_config == true and .proof_scope == "client:pg-agent-a" and .artifact_id == $artifact_id)
' >/dev/null
api_get "/api/v1/backup-artifacts?limit=10" | jq -e --arg artifact_id "$artifact_id" '
...
api_get "/api/v1/restore-plans?limit=10" | jq -e --arg restore_id "$restore_id" --arg backup_id "$backup_id" '
  any(.[]; .id == $restore_id and .source_backup_request_id == $backup_id and .source_client_id == "pg-agent-a" and .target_client_id == "pg-agent-b" and .status == "planned_metadata_only" and .destination_root == "/restore" and .proof_scope == "client:pg-agent-b")
' >/dev/null
api_get "/api/v1/jobs/$rejected_job_id" | jq -e --arg job_id "$rejected_job_id" '
  .id == $job_id and .command_type == "api_job_request" and .status == "rejected_authorization_required" and .target_count == 2
' >/dev/null
...
jq -n \
  ...
  '{
    ...
    api_url: $api_url,
    checks: ["auth_session", "agents", "postgres_reenrollment_key_rotation", "client_key_revocation", "telemetry_rollups", "telemetry_network_rates", "telemetry_pruning", "pool_tag_bulk", "tunnel_plan", "schedule", "backup_policy", "backup_policy_retention_prune", "worker_leases", "alert_notification_worker", "job_cancellation", "rejected_job_targets", "capability_degraded_update", "capability_degraded_process_limit", "backup_artifact_metadata", "backup_restore_metadata", "audit", "api_restart"]
  }'
