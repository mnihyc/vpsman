...
smoke_enter_root
smoke_require_tools awk base64 cp curl docker grep jq sed shuf timeout
...
wait_agent_connected() {
  ...
  until [[ "$status" == "connected" ]]; do
    if (( SECONDS >= deadline )); then
      smoke_dump_logs "agent did not connect for live hot-config smoke" \
...
if [[ "$reject_status" != "403" ]]; then
  echo "expected no-proof hot-config to return 403, got $reject_status" >&2
  cat "$reject_json" >&2 || true
  exit 1
fi
grep -q "display_name = \"$client_id\"" "$agent_config"
if grep -q "$updated_display_name" "$agent_config"; then
  echo "hot config changed after no-proof rejection" >&2
  exit 1
fi
