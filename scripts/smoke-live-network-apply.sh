...
smoke_enter_root
smoke_require_tools awk base64 cmp curl docker grep jq sed sha256sum shuf timeout
...
wait_agent_connected() {
  ...
  until [[ "$status" == "connected" ]]; do
    if (( SECONDS >= deadline )); then
      smoke_dump_logs "agent did not connect for live network-apply smoke" \
...
assert_outputs_redacted() {
  ...
  if grep -F \
    ...
    echo "network apply/rollback outputs leaked proof or trust material" >&2
    exit 1
  fi
}
...
assert_audit_evidence() {
  ...
  jq -e '[.[].action] |
    index("job.rejected_authorization_required")
    and index("job.dispatch_requested")
...
if [[ "$reject_status" != "403" ]]; then
  echo "expected no-proof network apply to return 403, got $reject_status" >&2
