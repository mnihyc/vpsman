...
smoke_enter_root
smoke_require_tools awk base64 cmp curl docker grep jq ping ps sha256sum shuf stat timeout
...
wait_agent_connected() {
  ...
  until [[ "$status" == "connected" ]]; do
    if (( SECONDS >= deadline )); then
      smoke_dump_logs "agent did not connect for postgres live job smoke" \
...
assert_gateway_session_ended() {
  ...
  jq -e --arg client "$expected_client" '
    any(.[]; .id == $client and .status == "disconnected")
...
assert_ospf_update_plans() {
  ...
  jq -e --arg client "$client_id" --arg peer "$peer_client_id" '
    any(.[]; .plan_name == "postgres-live-status"
      ...
      and .proof_required == (.cost_delta != 0)
...
assert_user_sessions_no_proof_rejection() {
  local reject_body reject_json reject_status audits_json
  reject_body="$(jq -n \
  ...
  if [[ "$reject_status" != "403" ]]; then
    echo "expected no-proof user-sessions to return 403, got $reject_status" >&2
  ...
  audits_json="$(api_get "/api/v1/audit?limit=30")"
  jq -e '[.[].action] | index("job.rejected_authorization_required")' \
    <<<"$audits_json" >/dev/null
}
