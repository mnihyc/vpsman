...
smoke_enter_root
smoke_require_tools awk base64 cp curl docker grep jq timeout
...
wait_agent_connected() {
  ...
  until [[ "$status" == "connected" ]]; do
    if (( SECONDS >= deadline )); then
      smoke_dump_logs "agent did not connect for live data-source patch smoke" \
...
if [[ "$reject_status" != "403" ]]; then
  echo "expected no-proof data-source config patch to return 403, got $reject_status" >&2
  cat "$reject_json" >&2 || true
  exit 1
fi
grep -q 'proc_root = "/proc"' "$agent_config"
if grep -q "$patch_proc_root" "$agent_config"; then
  echo "data-source patch changed config after no-proof rejection" >&2
  exit 1
fi
...
if ! jq -e '.accepted_targets == 1 and .status == "completed"' <<<"$push_json" >/dev/null; then
  echo "expected proof-gated data-source patch to complete; got:" >&2
  echo "$push_json" >&2
  dump_job_diagnostics "proof-gated data-source patch did not complete" "$job_id"
  exit 1
fi
