...
rotation_signing_keys="$(target/debug/vpsctl signing-keygen)"
rotation_public_hex="$(jq -r '.public_key_hex' <<<"$rotation_signing_keys")"
...
start_api() {
  ...
  VPSMAN_DISCOVERY_TRUSTED_SERVER_PUBLIC_KEYS_HEX="$rotation_public_hex" \
...
if [[ -z "$client_id" ]]; then
  smoke_fail "enroll-config did not write client_id for signed discovery failover smoke"
fi

sed -i "0,/tcp_addr = .*/s//tcp_addr = \"$dead_addr\"/" "$agent_config"
grep -q "discovery_url = \"$discovery_url\"" "$agent_config"
grep -q "server_ed25519_public_key_hex" "$agent_config"
grep -q "$rotation_public_hex" "$agent_config"
...
until [[ "$status" == "online" ]]; do
  if (( SECONDS >= deadline )); then
    smoke_dump_logs "agent did not become online through signed discovery failover" \
...
jq -e \
  ...
  '.version == 1 and (.signature | length == 64) and .endpoints[0].tcp_addr == $gateway_addr' \
...
jq -n \
  ...
  '{
    signed_discovery_failover_smoke: "ok",
    client_id: $client_id,
    dead_configured_endpoint: $dead_addr,
    discovered_endpoint: $discovered_addr,
    checks: ["https_or_localhost_discovery", "signed_document", "rotation_key_enrolled", "api_restart", "dead_tcp_failover"]
  }'
