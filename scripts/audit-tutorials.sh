...
grep -q 'VPSMAN_SUPER_PASSWORD' "$index_file" || fail "tutorial index must document local proof environment"
...
for file in "${expected_files[@]}"; do
  ...
  case "$file" in
    00-operator-quickstart.md)
      ...
      ;;
    02-enroll-agents.md)
      grep -qi 're-enroll' "$path" || fail "$path must cover rebuilt VPS re-enrollment"
      grep -qi 'unprivileged' "$path" || fail "$path must cover unprivileged agent mode"
      ;;
    05-data-source-presets.md)
