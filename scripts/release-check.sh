...
run_step repo-hygiene bash scripts/scan-repo-hygiene.sh
run_step large-file-audit bash scripts/audit-large-files.sh
run_step tutorial-audit bash scripts/audit-tutorials.sh
...
if [[ "${VPSMAN_RELEASE_SKIP_SMOKES:-0}" == "1" ]]; then
  ...
  skip_step smoke-signed-discovery-failover "VPSMAN_RELEASE_SKIP_SMOKES=1"
  ...
  run_step smoke-signed-discovery-failover \
    env VPSMAN_SMOKE_SKIP_BUILD=1 bash scripts/smoke-signed-discovery-failover.sh
