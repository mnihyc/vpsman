...
render_dev_config() {
  ...
  : "${VPSMAN_CLIENT_ID:?set VPSMAN_CLIENT_ID for development config}"
  cat <<EOF
client_id = "$VPSMAN_CLIENT_ID"
display_name = "$VPSMAN_CLIENT_ID"
...
render_enrolled_config() {
  ...
  : "${VPSMAN_CLIENT_ID:?set VPSMAN_CLIENT_ID for target-side enrollment}"
  : "${VPSMAN_SUPER_PASSWORD:?set VPSMAN_SUPER_PASSWORD for local proof-key derivation}"
  ...
  local args=(
    ...
    enroll-config
    --client-id "$VPSMAN_CLIENT_ID"
    --command-timeout-secs "$command_timeout_secs"
    --output-file "$tmp_config"
  )
  if [[ -n "${VPSMAN_SUPER_SALT_HEX:-}" ]]; then
    args+=(--super-salt-hex "$VPSMAN_SUPER_SALT_HEX")
  fi

  if ! VPSMAN_ENROLLMENT_TOKEN="$VPSMAN_ENROLLMENT_TOKEN" \
    VPSMAN_SUPER_PASSWORD="$VPSMAN_SUPER_PASSWORD" \
    "$helper" "${args[@]}"; then
    rm -f "$helper"
    return 1
  fi
