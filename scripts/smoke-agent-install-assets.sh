...
if [[ "${VPSMAN_SUPER_PASSWORD:-}" != "local-super-password" ]]; then
  echo "missing local super password env" >&2
  exit 1
fi
if [[ "${VPSMAN_SUPER_SALT_HEX:-}" != "01020304" ]]; then
  echo "missing super salt env" >&2
  exit 1
fi
...
test -n "$output"
test "$client_id" = "target-enrolled"
...
[auth]
proof_key_hex = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"
...
[auth]
proof_key_hex = "3333333333333333333333333333333333333333333333333333333333333333"
...
[auth]
proof_key_hex = "7777777777777777777777777777777777777777777777777777777777777777"
...
VPSMAN_INSTALL_ROOT="$enroll_root" \
  ...
  VPSMAN_CLIENT_ID="target-enrolled" \
...
grep -q 'proof_key_hex = ' "$enroll_root/etc/vpsman/agent.toml"
