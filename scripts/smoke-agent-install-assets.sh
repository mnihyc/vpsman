...
grep -q '^Environment=VPSMAN_AGENT_RESTART_MODE=signal_only$' "$systemd_unit"
grep -q "^Environment=VPSMAN_AGENT_STATE_DIR=$root_work_dir/state$" "$systemd_unit"
grep -q '^Restart=always$' "$systemd_unit"
grep -q '^Environment=VPSMAN_AGENT_STOP_MODE=restart_prevent_exit_64$' "$systemd_unit"
grep -q '^SuccessExitStatus=64$' "$systemd_unit"
grep -q '^RestartPreventExitStatus=64$' "$systemd_unit"
...
grep -q '^Environment=VPSMAN_AGENT_RESTART_MODE=signal_only$' "$unprivileged_unit"
grep -q "^Environment=VPSMAN_AGENT_STATE_DIR=$unprivileged_work_dir/state$" "$unprivileged_unit"
grep -q '^Environment=VPSMAN_AGENT_STOP_MODE=restart_prevent_exit_64$' "$unprivileged_unit"
grep -q '^SuccessExitStatus=64$' "$unprivileged_unit"
grep -q '^RestartPreventExitStatus=64$' "$unprivileged_unit"
