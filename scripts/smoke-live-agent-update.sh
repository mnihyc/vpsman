...
smoke_enter_root
smoke_require_tools awk base64 chmod cmp curl docker grep jq openssl sed sha256sum shuf stat timeout
...
cp target/release/vpsman-agent "$artifact_file"
...
wait_agent_connected() {
  ...
  until [[ "$status" == "connected" ]]; do
    if (( SECONDS >= deadline )); then
      smoke_dump_logs "agent did not connect for live agent-update smoke" \
...
assert_update_persisted() {
  ...
  jq -e '[.[].action] | index("job.rejected_authorization_required") and index("job.dispatch_requested") and index("job.target_result")' \
  ...
  if grep -F \
    ...
    echo "agent-update outputs leaked artifact URL, proof material, or trust anchors" >&2
    exit 1
  fi
}
...
assert_update_activation_failed() {
  ...
  if grep -F \
    ...
    echo "failed activation outputs leaked artifact URL, proof material, or trust anchors" >&2
    exit 1
  fi
}

wait_delegated_rollback_for_failed_activation() {
  local failed_rollout_id="$1"
  local failed_activate_job_id="$2"
  local failed_rollback_sha="$3"
  local delegated_rollback_job_id=""
  local deadline=$((SECONDS + 45))

  until delegated_rollback_job_id="$(api_get "/api/v1/agent-update-rollouts?limit=20" | jq -er \
    --arg rollout "$failed_rollout_id" \
    --arg client "$client_id" \
    --arg sha "$artifact_sha" '
    first(.[] | select(.id == $rollout
      ...
      and (.targets | length == 1)
      and .targets[0].client_id == $client
      and .targets[0].status == "rolled_back"
      and .rollback_delegations[0].force_unprivileged == true
      and .rollback_delegations[0].dispatched_count == 1)
      | .rollback_delegations[0].dispatch_job_id)
  ' 2>/dev/null)"; do
    if (( SECONDS >= deadline )); then
      smoke_dump_logs "delegated rollback was not dispatched after activation failure" \
        "$SMOKE_TMPDIR"/api-*.log "$gateway_log" "$SMOKE_TMPDIR"/agent-*.log "$https_log"
      api_get "/api/v1/agent-update-rollouts?limit=20" >&2 || true
      api_get "/api/v1/jobs?limit=20" >&2 || true
      exit 1
    fi
    sleep 0.5
  done

  api_get "/api/v1/jobs/$delegated_rollback_job_id" | jq -e \
    '.status == "completed" and .command_type == "agent_update_rollback" and .target_count == 1' \
    >/dev/null
  api_get "/api/v1/jobs/$delegated_rollback_job_id/targets" | jq -e --arg client "$client_id" '
    length == 1
      and .[0].client_id == $client
  ...
  api_get "/api/v1/audit?limit=100" | jq -e \
    --arg rollback_job "$delegated_rollback_job_id" \
    --arg activate_job "$failed_activate_job_id" \
    --arg client "$client_id" \
  ...
  printf '%s' "$delegated_rollback_job_id"
}
...
if [[ "$reject_status" != "403" ]]; then
  echo "expected no-proof agent-update to return 403, got $reject_status" >&2
  cat "$reject_json" >&2 || true
  exit 1
fi
if [[ -e "$staged_agent" || -e "$rollback_agent" ]]; then
  echo "agent update staged files after no-proof rejection" >&2
  exit 1
fi
...
rollback_delegation_json="$(VPSMAN_SUPER_PASSWORD="$super_password" \
VPSMAN_API_TOKEN="$access_token" \
  target/debug/vpsctl --api-url "$api_url" agent-update-rollout-delegate-rollback \
...
jq -e \
  ...
  --arg sha "$failed_rollback_sha" '
  .rollout_id == $rollout
    and .action == "agent_update_rollback"
    and .rollback_sha256_hex == $sha
    and .target_count == 1
    and .ready_count == 1
    and .dispatching_count == 0
    and .dispatched_count == 0
    and .force_unprivileged == true
' <<<"$rollback_delegation_json" >/dev/null
...
delegated_failure_rollback_job_id="$(
  wait_delegated_rollback_for_failed_activation \
    "$failed_rollout_id" \
    "$failed_activate_job_id" \
    "$failed_rollback_sha"
...
jq -n \
  ...
  --arg delegated_failure_rollback_job_id "$delegated_failure_rollback_job_id" \
  ...
  '{
    ...
    delegated_failure_rollback: "dispatched_and_rolled_back",
    client_id: $client_id,
    job_id: $job_id,
    ...
    delegated_failure_rollback_job_id: $delegated_failure_rollback_job_id,
