...
commands=(
  ...
  job-targets job-outputs job-follow job-output-artifact server-jobs server-job-cancel artifact-cleanup-preview artifact-cleanup-create audit history-retention history-retention-upsert history-retention-prune history-export network-observations network-trends network-ospf-recommendations network-ospf-update-plans topology-graph
...
workflow_for_command() {
  ...
  case "$command" in
    ...
    data-source-presets|data-source-preset-create|data-source-preset-clone|data-source-preset-diff|data-source-preset-test|data-source-preset-update|data-source-status|data-source-assignments|data-source-hot-config|data-source-hot-config-apply|data-source-preset-assign)
      ...
      ;;
    jobs|job-create|job-shell|job-targets|job-outputs|job-follow|job-output-artifact|server-jobs|server-job-cancel|artifact-cleanup-preview|artifact-cleanup-create)
    ...
    backups|backup-artifacts|backup-policies|backup-policy-upsert|backup-policy-prune|restore-plans|migration-links|backup-request|backup-run|backup-artifact-record|backup-artifact-upload|backup-artifact-upload-chunked|backup-artifact-handoff|restore-plan|migration-link|migration-run|restore-run|restore-rollback)
      ...
      ;;
    network-observations|network-trends|network-ospf-recommendations|network-ospf-update-plans|topology-graph|tunnel-plans|tunnel-plan|tunnel-promote-telemetry|tunnel-promote-adapter|tunnel-apply|tunnel-ospf-cost-update|tunnel-rollback|tunnel-status|tunnel-probe|tunnel-speed-test)
