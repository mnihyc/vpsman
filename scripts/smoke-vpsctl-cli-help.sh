...
commands=(
  ...
  host-process-refresh host-processes host-service-refresh host-services host-service-logs host-service-action host-storage-refresh host-storage os-update-check os-update-refresh os-update-plans os-update-plan os-update-apply process-start process-stop process-restart process-status process-logs process-supervisor-inventory
  job-targets job-target-status-download job-outputs job-follow job-output-download server-jobs server-job-cancel artifact-cleanup-preview artifact-cleanup-create audit history-retention history-retention-upsert history-retention-prune history-export network-observations network-trends network-ospf-recommendations network-ospf-update-plans topology-graph network-traffic-import-vnstat
  ...
  bulk-resolve tunnel-plans port-forwards port-forward-create port-forward-update port-forward-enable port-forward-disable port-forward-delete port-forward-forget port-forward-reapply port-forward-resolve port-forward-bulk tunnel-allocate tunnel-plan tunnel-plan-export tunnel-plan-enable tunnel-plan-disable tunnel-plan-rotate-credentials tunnel-plan-delete tunnel-ospf-status-refresh tunnel-ospf-cost-update
...
workflow_for_command() {
  ...
  case "$command" in
    ...
    file-pull|file-push|file-transfer-upload|file-transfer-download|file-transfers|file-transfer-handoff|file-transfer-sources|file-transfer-source-upload|file-transfer-source-download)
      ...
      ;;
    user-sessions|process-start|process-stop|process-restart|process-status|process-logs|process-supervisor-inventory)
      printf 'process_and_session_inventory\n'
      ;;
    host-process-refresh|host-processes|host-service-refresh|host-services|host-service-logs|host-service-action|host-storage-refresh|host-storage|os-update-check|os-update-refresh|os-update-plans|os-update-plan|os-update-apply)
      printf 'host_management\n'
      ;;
    config-patch|agent-update|agent-update-check|agent-update-activate|agent-update-rollback|agent-update-release-record|agent-update-release-latest|agent-update-releases)
    ...
    backups|backup-artifacts|backup-policies|backup-policy-upsert|backup-policy-prune|restore-plans|migration-links|backup-request|backup-run|backup-artifact-record|backup-artifact-upload|backup-artifact-upload-chunked|backup-artifact-handoff|restore-plan|migration-link|migration-run|restore-run|restore-rollback)
      ...
      ;;
    network-observations|network-trends|network-ospf-recommendations|network-ospf-update-plans|topology-graph|network-traffic-import-vnstat|tunnel-plans|port-forwards|port-forward-create|port-forward-update|port-forward-enable|port-forward-disable|port-forward-delete|port-forward-forget|port-forward-reapply|port-forward-resolve|port-forward-bulk|tunnel-allocate|tunnel-plan|tunnel-plan-export|tunnel-plan-enable|tunnel-plan-disable|tunnel-plan-rotate-credentials|tunnel-plan-delete|tunnel-ospf-status-refresh|tunnel-ospf-cost-update|tunnel-status|tunnel-probe|tunnel-speed-test)
