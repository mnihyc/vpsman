...
require_no_match \
  "server audit/repository code must not persist proof material fields" \
  'proof_hex|PrivilegeProof|superPassword|super_password' \
  crates/api/src/repository*.rs migrations

require_match \
  "proof replay cache and replay error are implemented" \
...
printf '  "checks": ["no_server_plaintext_super_password", "no_repository_proof_material_persistence", "proof_replay_cache", "object_key_safety", "operator_password_hashing"]\n'
printf '}\n'
