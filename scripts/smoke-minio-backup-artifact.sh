...
smoke_start_postgres "vpsman-minio-backup-postgres" "$pg_port" >/dev/null
...
printf 'synthetic plain backup bytes for MinIO smoke %s\n' "$(date +%s%N)" >"$payload_file"
...
object_key="backups/$client_id/$backup_request_id.tar"
...
downloaded="$SMOKE_TMPDIR/downloaded-artifact.tar"
curl -fsS \
