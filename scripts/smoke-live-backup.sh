...
smoke_enter_root
smoke_require_tools base64 curl grep jq sha256sum shuf timeout
...
until [[ "$status" == "connected" ]]; do
  if (( SECONDS >= deadline )); then
    smoke_dump_logs "agent did not connect for live backup smoke" \
...
if [[ "$reject_status" != "403" ]]; then
  echo "expected no-proof backup to return 403, got $reject_status" >&2
...
if grep -Fq "proof_key_hex" <<<"$artifact_json"; then
  echo "backup artifact leaked agent config plaintext" >&2
  exit 1
fi
