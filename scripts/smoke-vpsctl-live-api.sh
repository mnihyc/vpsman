...
smoke_enter_root
smoke_require_tools bash cargo curl docker jq shuf timeout

fail() {
  echo "$1" >&2
  if [[ -n "${api_log:-}" && -f "$api_log" ]]; then
    smoke_dump_logs "vpsctl live API smoke failed" "$api_log"
  fi
  exit 1
}

if [[ "${VPSMAN_SMOKE_SKIP_BUILD:-0}" != "1" ]]; then
  cargo build -p vpsman-api -p vpsctl >/dev/null
fi
...
viewer_job_response="$(curl -sS -w '\n%{http_code}' \
  ...
  -H "Content-Type: application/json" \
  -d '{"command":"/bin/true","argv":[],"clients":["cli-agent-a"],"pools":[],"tags":[],"privileged":true,"destructive":false,"confirmed":true,"timeout_secs":1,"envelope":null,"envelopes":{}}' \
  "$api_url/api/v1/jobs")"
...
scoped_pool_response="$(curl -sS -w '\n%{http_code}' \
  -H "Authorization: Bearer $scoped_access_token" \
  -H "Content-Type: application/json" \
  -d '{"name":"scope-denied-pool","provider":null,"region":null}' \
  "$api_url/api/v1/pools")"
scoped_pool_status="${scoped_pool_response##*$'\n'}"
scoped_pool_body="${scoped_pool_response%$'\n'*}"
if [[ "$scoped_pool_status" != "403" ]]; then
  fail "fleet:read scoped pool create returned HTTP $scoped_pool_status: $scoped_pool_body"
fi
jq -e '.error == "operator_scope_insufficient"' <<<"$scoped_pool_body" >/dev/null
...
jq -e '.total == 2 and .connected == 2' <<<"$summary_json" >/dev/null
agents_json="$(vpsctl_auth agents)"
jq -e 'length == 2 and any(.[]; .id == "cli-agent-a" and .capabilities.privilege_mode == "root") and any(.[]; .id == "cli-agent-b" and .capabilities.privilege_mode == "unprivileged" and .capabilities.can_apply_process_limits == false)' \
  <<<"$agents_json" >/dev/null
data_source_status_json="$(vpsctl_auth data-source-status --domain telemetry_metrics_source)"
jq -e 'length == 2 and all(.[]; .domain == "telemetry_metrics_source" and .preset_name == "builtin:linux_procfs" and .status == "selected")' \
  <<<"$data_source_status_json" >/dev/null
backup_source_status_json="$(vpsctl_auth data-source-status --domain backup_object_store)"
jq -e '
  length == 2 and
  all(.[]; .domain == "backup_object_store" and .preset_name == "builtin:local_filesystem" and .status == "selected_no_store") and
  all(.[]; .evidence.server_object_store_configured == false and (.evidence.artifact_count | type == "number"))
' <<<"$backup_source_status_json" >/dev/null
update_source_status_json="$(vpsctl_auth data-source-status --domain update_artifact_source)"
jq -e '
  length == 2 and
  all(.[]; .domain == "update_artifact_source" and .preset_name == "builtin:local_filesystem_or_https" and .status == "selected_no_artifacts") and
  all(.[]; .evidence.server_object_store_configured == false and .evidence.release_count == 0)
...
jq -e '
  ...
  any(.[]; .client_id == "cli-agent-a" and .domain == "process_inventory_source" and .status == "ready_on_demand" and .evidence.workflow == "process_inventory" and .evidence.proof_gated == true and .evidence.supervisor_workflow == "process_supervisor" and .evidence.process_limits_status == "available") and
...
pool_json="$(vpsctl_auth pool-create --name cli-pool --provider cli-provider --region cli-region)"
pool_id="$(jq -r '.id' <<<"$pool_json")"
jq -e '.name == "cli-pool" and .provider == "cli-provider" and .region == "cli-region"' \
  <<<"$pool_json" >/dev/null
vpsctl_auth agent-pool --client-id cli-agent-a --pool-id "$pool_id" >/dev/null
...
schedule_json="$(vpsctl_auth schedule-create \
  ...
  --max-failures 7)"
jq -e '.name == "cli-hourly-uptime" and .enabled == true and .command_type == "shell_argv" and .tags == ["edge"] and .catch_up_policy == "run_once" and .retry_delay_secs == 120 and .max_failures == 7' \
...
jq -e '.client_id == "cli-agent-a" and .status == "requested_metadata_only" and .proof_scope == "client:cli-agent-a" and .include_config == true' \
...
jq -e --arg backup_id "$backup_id" '.source_backup_request_id == $backup_id and .source_client_id == "cli-agent-a" and .target_client_id == "cli-agent-b" and .status == "planned_metadata_only" and .proof_scope == "client:cli-agent-b"' \
