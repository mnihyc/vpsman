...
Use pools for provider/resource-pool hierarchy and tags for operational
selection. This keeps bulk actions predictable when managing 20+ VPSs.

## Create Pools And Tags

```sh
cargo run -p vpsctl -- pool-create --name provider-a-sfo --provider provider-a --region sfo
...
```sh
cargo run -p vpsctl -- agent-pool --client-id edge-01 --pool-id <pool_uuid>
...
cargo run -p vpsctl -- bulk-resolve --pools <pool_uuid>
cargo run -p vpsctl -- bulk-resolve --clients edge-01 --tags canary
```

Use explicit client ids for destructive one-off work, tags for operational
groups, and pools when the provider/resource-pool hierarchy matters.
...
For provider, pool, tag, or client-specific thresholds, save a scoped policy:
...
Scoped policies cascade from global to provider, pool, tag, and client matches.
...
In the panel, use the left navigation for fleet, pools/tags, jobs, topology,
backups, and updates. The UI is meant for repeated operations: filter first,
inspect exact targets, then dispatch.

## Operator Rules

- Treat tags as operational intent: `edge`, `bgp`, `canary`, `backup-critical`.
- Treat pools as infrastructure ownership: provider, region, resource pool, or
  reseller account.
