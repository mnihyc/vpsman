...
Proof-gated command execution resolves targets, builds local proof envelopes,
and sends only derived proof material to the server:
...
Small proof-gated pulls and pushes:
...
Create an approval-required schedule:
...
Approve a due scheduled job with fresh proof:
