...
Backups and restores are proof-gated workflows. Backup private key material
...
Create a policy for a client, pool, or tag selector. Due policies materialize as
approval-required scheduled backup jobs; approve those jobs with the normal
schedule dispatch flow and fresh proof. Once approved, the dispatched job
creates the same per-target backup request and encrypted artifact history as a
manual `backup-run`.
...
artifact locally, and dispatches the restore command with scoped proof. The API
does not receive the backup private key or plaintext super password.
