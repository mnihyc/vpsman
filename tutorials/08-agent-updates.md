...
Agent updates are proof-gated and staged. The default flow is publish metadata
or upload a hosted artifact, dispatch staging, then activate or roll back.
...
the server to dispatch privileged activation without operator proof. Use
`manual_after_canary` to stop after the first verified canary, and `manual_only`
when every promotion should be explicitly selected by an operator.

To let the server continue the worker-recommended canary and next-batch
activation path without repeated proof entry, delegate activation proof for the
rollout:

```sh
cargo run -p vpsctl -- agent-update-rollout-delegate-activation \
  --rollout-id <rollout_uuid> \
  --proof-ttl-secs 3600 \
  --restart-agent \
  --confirmed
...
Activation delegation locally creates per-client proof envelopes for the exact
`agent_update_activate` command bound to the rollout artifact SHA-256 and
restart preference. By default it escrows proofs for all rollout targets so
future canary/next-batch recommendations can be dispatched; use
`--clients edge-a,edge-b` to narrow the escrow. The API claims a proof only
when the worker recommends `operator_activate_batch`, the target is still
`completed`, and the staged hash still matches the rollout.
`agent-update-rollouts` returns activation and rollback delegation summaries
with ready, dispatching, dispatched, expired, and failed counts plus proof
expiry windows. Re-run the same delegation command to renew expired proofs; the
panel labels expired or failed proof rows as `Renew act.` or `Renew roll.`.
Add `--force-unprivileged` only when the delegated activation should be
attempted on known normal-user agents. The flag is stored as dispatch policy for
the exact proof envelope; it does not let the server rewrite the activation
payload.

To let the server automatically recover a canary that fails to send its
post-restart heartbeat, delegate rollback proof before or immediately after
activation:

```sh
cargo run -p vpsctl -- agent-update-rollout-delegate-rollback \
  --rollout-id <rollout_uuid> \
  --proof-ttl-secs 3600 \
  --confirmed
```

Delegation locally creates per-client proof envelopes for the exact
`agent_update_rollback` command and sends only those envelopes to the API. The
server cannot alter the command without changing the payload hash. It dispatches
the frozen rollback job only if the matching rollout target becomes
`heartbeat_timeout` or `activation_failed`. Use `--clients edge-a,edge-b` for a
subset and `--rollback-sha256-hex <64_hex_rollback_sha256>` to pin a rollback
binary hash. Add `--force-unprivileged` only when rollback should be attempted
on known normal-user agents.
...
state, health gate, worker lease evidence, and delegated rollback dispatch
results without storing artifact URLs, detached signatures, public signing
keys, plaintext super password, or trust anchors.
