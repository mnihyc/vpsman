    ...
    match parts.first().copied().context("agent update command is empty")? {
        "agent-update" => submit_vty_agent_update(
            ...
            parse_vty_agent_update(&parts[1..]).with_context(|| {
                "usage: agent-update --artifact-url <https-url> --sha256-hex <sha256> [--artifact-signature-hex <sig>] [--artifact-signing-key-hex <key>] [--canary-count <n>] <target ...> [--timeout <1-3600>] [--privilege-ttl <1-3600>] [--force-unprivileged] --confirmed"
            })?,
        ),
