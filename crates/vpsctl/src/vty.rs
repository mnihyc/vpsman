            ...
            command if is_vty_agent_update_rollout_policy_create_command(command) => {
                match submit_vty_agent_update_rollout_policy_create(
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: agent-update-rollout-policy-create --name <name> --scope-kind global|tag|provider [--scope-value <value>] [--channel <name>] [--canary-count <0-10000>] [--health-gate heartbeat_verified|manual_after_canary|manual_only] [--priority <n>] [--disabled] --confirmed"
                        );
                    }
                }
            }
