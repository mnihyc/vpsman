...
use crate::vty_update_rollouts::{
    ...
    submit_vty_agent_update_rollout_policies, submit_vty_agent_update_rollout_policy_create,
    submit_vty_agent_update_rollout_rollback, submit_vty_agent_update_rollouts,
};

pub(crate) fn run_vty(api_url: &str) -> Result<()> {
    println!("vpsman VTY connected to {api_url}");
    println!(
        "Commands: health, summary, agents, fleet-alerts, fleet-alert-export, fleet-alert-states, fleet-alert-state-update, fleet-alert-policies, fleet-alert-policy-upsert, fleet-alert-notification-channels, fleet-alert-notification-channel-upsert, fleet-alert-notifications, fleet-alert-notification-dispatch, fleet-alert-notification-process, operators, operator-create, operator-sessions, operator-session-revoke, totp-setup, totp-confirm, totp-disable, enrollment-tokens, enrollment-settings, enrollment-token-create, reenrollment-token-create, client-key-revocations, client-key-revoke, key-lifecycle-report, gateway-sessions, telemetry-rollups, telemetry-network-rates, telemetry-tunnels, tags, tag-create, agent-tag, data-source-presets, data-source-preset-create, data-source-preset-clone, data-source-preset-diff, data-source-preset-test, data-source-preset-update, data-source-status, data-source-assignments, data-source-hot-config, data-source-hot-config-apply, data-source-preset-assign, jobs, job-cancel, schedules, schedule-create, job-create, job-shell, terminal-open, terminal-input, terminal-poll, terminal-resize, terminal-close, terminal-sessions, terminal-replay, terminal-follow, file-pull, file-push, file-transfer-upload, file-transfer-download, file-transfers, file-transfer-handoff, file-transfer-sources, file-transfer-source-upload, file-transfer-source-download, user-sessions, hot-config, agent-update, agent-update-activate, agent-update-rollback, agent-update-rollouts, agent-update-rollout-policies, agent-update-rollout-policy-create, agent-update-rollout-activate, agent-update-rollout-rollback, agent-update-rollout-control, agent-update-releases, agent-update-release-record, agent-update-artifact-upload, process-list, process-start, process-stop, process-restart, process-status, process-logs, process-supervisor-inventory, job-targets, job-outputs, job-follow, job-output-artifact, backups, backup-policies, backup-artifacts, backup-policy-upsert, backup-policy-prune, backup-request, backup-run, backup-artifact-record, backup-artifact-upload, backup-artifact-upload-chunked, backup-artifact-handoff, restore-plans, restore-plan, restore-run, restore-rollback, migration-links, migration-link, migration-run, tunnel-plans, tunnel-plan, tunnel-promote-telemetry, tunnel-promote-adapter, tunnel-apply, tunnel-ospf-cost-update, tunnel-rollback, tunnel-status, tunnel-probe, tunnel-speed-test, network-observations, network-trends, network-ospf-recommendations, network-ospf-update-plans, topology-graph, audit, history-retention, history-retention-upsert, history-retention-prune, history-export, bulk-resolve, enable, signing-keygen, exit"
    );
            ...
            command if command.starts_with("file-transfer-upload ") => {
                ...
                let request = match parse_vty_file_transfer_upload(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: file-transfer-upload (--source <local-file>|--source-artifact-id <uuid>) --path <remote-abs> <target ...> [--mode <0644>] [--session-id <uuid>] [--resume-token <token>] [--chunk-size-bytes <1-65536>] [--rate-limit-kbps <0-1000000>] [--multi-target-policy same-offset|independent-offsets] [--timeout <1-3600>] [--privilege-ttl <1-3600>] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("file-transfer-download ") => {
                ...
                let request = match parse_vty_file_transfer_download(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: file-transfer-download --path <remote-abs> --destination <local-file-or-dir> <target ...> [--session-id <uuid>] [--resume-token <token>] [--chunk-size-bytes <1-65536>] [--rate-limit-kbps <0-1000000>] [--multi-target-policy single-target|per-target-files] [--timeout <1-3600>] [--privilege-ttl <1-3600>] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("hot-config ") => {
                ...
                let request = match parse_vty_hot_config(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: hot-config --config-file <path> <target ...> [--timeout <1-3600>] [--privilege-ttl <1-3600>] [--force-unprivileged] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("data-source-hot-config-apply ") => {
                ...
                let request = match parse_vty_data_source_hot_config_apply(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: data-source-hot-config-apply --client-id <id> [--timeout <1-3600>] [--privilege-ttl <1-3600>] [--force-unprivileged] --confirmed"
                        );
                        continue;
                    }
                };
