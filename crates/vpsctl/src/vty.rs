...
use crate::vty_enrollment::{is_vty_enrollment_command, submit_vty_enrollment_command};
...
use crate::vty_jobs::{
    ...
    VtyJobSelection, VtyProofContext,
};
...
use crate::vty_schedules::{
    parse_vty_schedule_create_options, parse_vty_schedule_dispatch, submit_vty_schedule_create,
    submit_vty_schedule_dispatch,
};
...
use crate::vty_update_rollouts::{
    ...
    submit_vty_agent_update_rollout_activate, submit_vty_agent_update_rollout_control,
    submit_vty_agent_update_rollout_delegate_activation,
    submit_vty_agent_update_rollout_delegate_rollback, submit_vty_agent_update_rollout_policies,
    submit_vty_agent_update_rollout_policy_create, submit_vty_agent_update_rollout_rollback,
    submit_vty_agent_update_rollouts,
};

pub(crate) fn run_vty(api_url: &str) -> Result<()> {
    println!("vpsman VTY connected to {api_url}");
    println!(
        "Commands: health, summary, agents, fleet-alerts, fleet-alert-export, fleet-alert-states, fleet-alert-state-update, fleet-alert-policies, fleet-alert-policy-upsert, fleet-alert-notification-channels, fleet-alert-notification-channel-upsert, fleet-alert-notifications, fleet-alert-notification-dispatch, fleet-alert-notification-process, operators, operator-create, operator-sessions, operator-session-revoke, totp-setup, totp-confirm, totp-disable, enrollment-tokens, enrollment-token-create, reenrollment-token-create, client-key-revocations, client-key-revoke, key-lifecycle-report, super-password-rotations, gateway-sessions, telemetry-rollups, telemetry-network-rates, telemetry-tunnels, pools, pool-create, agent-pool, tags, tag-create, agent-tag, data-source-presets, data-source-preset-create, data-source-preset-clone, data-source-preset-diff, data-source-preset-test, data-source-preset-update, data-source-status, data-source-assignments, data-source-hot-config, data-source-hot-config-apply, data-source-preset-assign, jobs, job-cancel, schedules, schedule-create, schedule-dispatch, job-create, job-shell, terminal-open, terminal-input, terminal-poll, terminal-resize, terminal-close, terminal-sessions, terminal-replay, terminal-follow, file-pull, file-push, file-transfer-upload, file-transfer-download, file-transfers, file-transfer-handoff, file-transfer-sources, file-transfer-source-upload, file-transfer-source-download, user-sessions, hot-config, agent-update, agent-update-activate, agent-update-rollback, agent-update-rollouts, agent-update-rollout-policies, agent-update-rollout-policy-create, agent-update-rollout-activate, agent-update-rollout-rollback, agent-update-rollout-delegate-rollback, agent-update-rollout-delegate-activation, agent-update-rollout-control, agent-update-releases, agent-update-release-record, agent-update-artifact-upload, process-list, process-start, process-stop, process-restart, process-status, process-logs, process-supervisor-inventory, job-targets, job-outputs, job-follow, job-output-artifact, backups, backup-policies, backup-artifacts, backup-policy-upsert, backup-policy-prune, backup-request, backup-run, backup-artifact-record, backup-artifact-upload, backup-artifact-upload-chunked, backup-artifact-handoff, restore-plans, restore-plan, restore-run, restore-rollback, migration-links, migration-link, migration-run, tunnel-plans, tunnel-plan, tunnel-promote-telemetry, tunnel-promote-adapter, tunnel-apply, tunnel-ospf-cost-update, tunnel-rollback, tunnel-status, tunnel-probe, tunnel-speed-test, network-observations, network-trends, network-ospf-recommendations, network-ospf-update-plans, topology-graph, audit, history-retention, history-retention-upsert, history-retention-prune, history-export, bulk-resolve, enable, proof, signing-keygen, exit"
    );
    println!("{}", vty_privilege_help());
    let stdin = io::stdin();
    let mut proof_context = VtyProofContext::default();
    ...
    loop {
        ...
        match command {
            ...
            command if is_vty_enrollment_command(command) => {
                match submit_vty_enrollment_command(api_url, token.as_deref(), command) {
                    Ok(output) => println!("{output}"),
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: reenrollment-token-create --client-id <id> [--ttl <secs>] [--default-tags a,b] --confirmed | client-key-revoke --client-id <id> [--reason text] --confirmed"
                        );
                    }
                }
            }
            ...
            command if is_vty_agent_update_rollouts_command(command) => println!(
                "{}",
                submit_vty_agent_update_rollouts(api_url, token.as_deref(), command)?
            ),
            command if is_vty_agent_update_rollout_policies_command(command) => println!(
                "{}",
                submit_vty_agent_update_rollout_policies(api_url, token.as_deref(), command)?
            ),
            command if is_vty_agent_update_rollout_policy_create_command(command) => {
                match submit_vty_agent_update_rollout_policy_create(
                    api_url,
                    token.as_deref(),
                    command,
                ) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: agent-update-rollout-policy-create --name <name> --scope-kind global|tag|pool|provider [--scope-value <value>] [--channel <name>] [--canary-count <0-10000>] [--health-gate heartbeat_verified|manual_after_canary|manual_only] [--priority <n>] [--disabled] --confirmed"
                        );
                    }
                }
            }
            command if is_vty_agent_update_rollout_activate_command(command) => {
                if !proof_context.enabled {
                    println!(
                ...
                println!(
                    "{}",
                    submit_vty_agent_update_rollout_activate(
                        api_url,
                        token.as_deref(),
            ...
            command if is_vty_agent_update_rollout_rollback_command(command) => {
                if !proof_context.enabled {
                    println!(
                        "privileged proof is required; run enable after setting VPSMAN_SUPER_PASSWORD and VPSMAN_SUPER_SALT_HEX"
                    );
                ...
                println!(
                    "{}",
                    submit_vty_agent_update_rollout_rollback(
                        api_url,
                        token.as_deref(),
                        &proof_context.password,
                        &proof_context.salt_hex,
            ...
            command if is_vty_agent_update_rollout_delegate_rollback_command(command) => {
                if !proof_context.enabled {
                    println!(
                        "privileged proof is required; run enable after setting VPSMAN_SUPER_PASSWORD and VPSMAN_SUPER_SALT_HEX"
                    );
                ...
                println!(
                    "{}",
                    submit_vty_agent_update_rollout_delegate_rollback(
                        api_url,
                        token.as_deref(),
                        &proof_context.password,
                        &proof_context.salt_hex,
            ...
            command if is_vty_agent_update_rollout_delegate_activation_command(command) => {
                if !proof_context.enabled {
                    println!(
                        "privileged proof is required; run enable after setting VPSMAN_SUPER_PASSWORD and VPSMAN_SUPER_SALT_HEX"
                    );
            ...
            command if is_vty_agent_update_rollout_control_command(command) => {
                match submit_vty_agent_update_rollout_control(api_url, token.as_deref(), command) {
                    Ok(output) => println!("{output}"),
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: agent-update-rollout-control --rollout-id <uuid> (--pause|--resume|--health-gate <heartbeat_verified|manual_after_canary|manual_only>) [--pause-reason <text>] --confirmed"
                        );
                    }
                }
            }
            ...
            command if command.starts_with("schedule-create ") => {
                let parts = command.split_whitespace().collect::<Vec<_>>();
                if parts.len() < 5 {
                    println!("usage: schedule-create <name> <interval_secs> <command> [schedule policy flags] <target ...>");
                    continue;
                }
                let interval_secs = match parts[2].parse::<u64>() {
                    Ok(value) => value,
                    Err(error) => {
                        println!("usage error: interval_secs must be an integer: {error}");
                        continue;
                    }
                };
                let schedule_options = match parse_vty_schedule_create_options(&parts[4..]) {
                    Ok(options) => options,
                    Err(error) => {
                        println!("usage error: {error}");
                        continue;
            ...
            command if command.starts_with("schedule-dispatch ") => {
                let parts = command.split_whitespace().collect::<Vec<_>>();
                if !proof_context.enabled {
                    println!(
                        "privileged proof is required; run enable after setting VPSMAN_SUPER_PASSWORD and VPSMAN_SUPER_SALT_HEX"
                ...
                let request = match parse_vty_schedule_dispatch(&parts[1..]) {
                    Ok(request) => request,
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: schedule-dispatch <job_uuid> [--timeout <1-3600>] [--proof-ttl <1-3600>] [--force-unprivileged] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("job-create ") => {
                ...
                if parts.len() < 3 {
                    ...
                    println!("targets: client:<id> pool:<uuid> tag:<name>; bare targets are tags");
                    continue;
                }
                if !proof_context.enabled {
                    println!(
            ...
            command if command.starts_with("file-transfer-upload ") => {
                ...
                let request = match parse_vty_file_transfer_upload(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: file-transfer-upload (--source <local-file>|--source-artifact-id <uuid>) --path <remote-abs> <target ...> [--mode <0644>] [--session-id <uuid>] [--resume-token <token>] [--chunk-size-bytes <1-65536>] [--rate-limit-kbps <0-1000000>] [--multi-target-policy same-offset|independent-offsets] [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("file-transfer-download ") => {
                ...
                let request = match parse_vty_file_transfer_download(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: file-transfer-download --path <remote-abs> --destination <local-file-or-dir> <target ...> [--session-id <uuid>] [--resume-token <token>] [--chunk-size-bytes <1-65536>] [--rate-limit-kbps <0-1000000>] [--multi-target-policy single-target|per-target-files] [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("hot-config ") => {
                ...
                let request = match parse_vty_hot_config(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: hot-config --config-file <path> <target ...> [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("data-source-hot-config-apply ") => {
                ...
                let request = match parse_vty_data_source_hot_config_apply(&parts[1..]) {
                    ...
                    Err(error) => {
                        println!("usage error: {error}");
                        println!(
                            "usage: data-source-hot-config-apply --client-id <id> [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed"
                        );
                        continue;
                    }
                };
            ...
            command if command.starts_with("backup-run ") => {
                ...
                println!(
                    "{}",
                    submit_vty_backup_run(api_url, token.as_deref(), &proof_context, request,)?
                );
            }
            ...
            command if command.starts_with("restore-run ") => {
                ...
                println!(
                    "{}",
                    submit_vty_restore_run(api_url, token.as_deref(), &proof_context, request)?
                );
            }
            ...
            command if command.starts_with("migration-run ") => {
                ...
                println!(
                    "{}",
                    submit_vty_migration_run(api_url, token.as_deref(), &proof_context, request,)?
                );
            }
            ...
            "enable" => match VtyProofContext::from_env() {
                Ok(context) => {
                    proof_context = context;
                    println!(
                        "privileged mode enabled locally; proof envelopes will be generated without sending the super password"
                    );
                }
                Err(error) => {
                    proof_context = VtyProofContext::default();
            ...
            "disable" => {
                proof_context = VtyProofContext::default();
                println!(
                    "privileged mode disabled; local proof material cleared for this VTY session"
                );
            }
            "show privilege" => println!("{}", render_vty_privilege_status(&proof_context)),
            "show capabilities" => println!("{}", render_vty_capabilities()),
            "show degraded-policy" => println!("{}", render_vty_degraded_policy()),
            "proof" => {
                println!(
                    "use: vpsctl proof --scope <scope> --salt-hex <hex> --payload-hash-hex <sha256>; VTY job-create and job-shell use VPSMAN_SUPER_PASSWORD and VPSMAN_SUPER_SALT_HEX after enable"
                );
            }
            "signing-keygen" => {
            ...
            "help" | "?" => {
                println!("{}", vty_privilege_help());
                println!(
                    "health | summary | agents | fleet-alerts [filters] | fleet-alert-export [filters] | fleet-alert-states [--state open|acknowledged|muted|escalated] | fleet-alert-state-update --alert-id <id> --action acknowledge|mute|escalate|clear [--muted-for-secs <secs>] [--reason <text>] --confirmed | fleet-alert-policies [--limit <1-1000>] [--enabled true|false] [--scope-kind global|provider|pool|tag|client] [--scope-value <value>] | fleet-alert-policy-upsert --name <name> --scope-kind global|provider|pool|tag|client [--scope-value <value>] [resource threshold flags] [--priority <n>] [--enabled true|false] [--notes <text>] --confirmed | fleet-alert-notification-channels [--limit <1-1000>] [--enabled true|false] [--scope-kind global|provider|pool|tag|client] [--scope-value <value>] [--delivery-kind <kind>] | fleet-alert-notification-channel-upsert --name <name> --scope-kind global|provider|pool|tag|client [--scope-value <value>] [--min-severity critical|warning|info] [--categories a,b] [--operator-states open,escalated] --delivery-kind <kind> --target <target> [--cooldown-secs <secs>] [--enabled true|false] [--notes <text>] --confirmed | fleet-alert-notifications [--limit <1-1000>] [--channel-id <uuid>] [--alert-id <id>] [--status <status>] | fleet-alert-notification-dispatch [filters] [--include-muted] [--dry-run|--confirmed] | fleet-alert-notification-process [--limit <1-200>] [--status queued|failed] [--delivery-kind <kind>] [--dry-run|--confirmed] | operators | operator-create <username> <admin|operator|viewer> <password_env> [scope,scope] | operator-sessions [--limit <1-200>] | operator-session-revoke <session_uuid> | super-password-rotations [--limit <1-200>] | totp-setup [password_env] | totp-confirm [password_env] [code_env] | totp-disable [password_env] [code_env] | gateway-sessions [--limit <1-200>] | telemetry-rollups [--limit <1-200>] [--client-id <id>] [--bucket-secs <60-86400>] | telemetry-network-rates [--limit <1-200>] [--client-id <id>] [--interface <name>] [--bucket-secs <60-86400>] | telemetry-tunnels [--limit <1-200>] [--client-id <id>] [--interface <name>] | pools | pool-create <name> [provider] [region] | agent-pool <client_id> <pool_uuid> | tags | tag-create <name> | agent-tag <client_id> <tag> | data-source-presets [--domain <domain>] | data-source-preset-create --domain <domain> --name <name> [--scope shared|vps_local] [--owner-client-id <client>] [--description <text>] [--definition-json <json>] | data-source-preset-clone --source-preset-id <uuid> --name <name> [--description <text>] | data-source-preset-diff --preset-id <uuid> [--description <text>|--clear-description] [--definition-json <json>] | data-source-preset-test --preset-id <uuid> [--definition-json <json>] | data-source-preset-update --preset-id <uuid> [--description <text>|--clear-description] [--definition-json <json>] [--confirmed] | data-source-status [--client-id <id>] [--domain <domain>] | data-source-assignments [--client-id <id>] [--domain <domain>] | data-source-hot-config --client-id <id> [--format toml|json] | data-source-hot-config-apply --client-id <id> [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | data-source-preset-assign --domain <domain> --preset-id <uuid> [--client <id>] [--pool <uuid>] [--tag <tag>] [--confirmed] | jobs | job-cancel <job_uuid> --confirmed [reason] | schedules | schedule-create <name> <interval_secs> <command> [--catch-up-policy skip_missed|run_once|run_all_limited] [--catch-up-limit <1-25>] [--retry-delay-secs <1-86400>] [--max-failures <1-100>] <target ...> | schedule-dispatch <job_uuid> [--timeout <1-3600>] [--proof-ttl <1-3600>] [--force-unprivileged] --confirmed | job-create <command> <client:id|pool:uuid|tag:name|tag ...> [--destructive --confirmed] | job-shell <shell-script> -- <client:id|pool:uuid|tag:name|tag ...> | file-pull --path <remote-abs> <target ...> [--timeout <1-3600>] [--confirmed] | file-push --source <local-file> --path <remote-abs> <target ...> [--mode <0644>] [--timeout <1-3600>] --confirmed | file-transfer-upload (--source <local-file>|--source-artifact-id <uuid>) --path <remote-abs> <target ...> [--mode <0644>] [--session-id <uuid>] [--resume-token <token>] [--chunk-size-bytes <1-65536>] [--rate-limit-kbps <0-1000000>] [--multi-target-policy same-offset|independent-offsets] [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | file-transfer-download --path <remote-abs> --destination <local-file-or-dir> <target ...> [--session-id <uuid>] [--resume-token <token>] [--chunk-size-bytes <1-65536>] [--rate-limit-kbps <0-1000000>] [--multi-target-policy single-target|per-target-files] [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | file-transfers [--limit <1-200>] [--client-id <id>] [--session-id <uuid>] | file-transfer-handoff --client-id <id> --session-id <uuid> [--output-file <file>] --confirmed | file-transfer-sources [--limit <1-200>] | file-transfer-source-upload --source <file> [--name <name>] --confirmed | file-transfer-source-download --artifact-id <uuid> --output-file <file> | user-sessions <target ...> [--timeout <1-3600>] [--confirmed] | hot-config --config-file <path> <target ...> [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | super-password-rotate (--new-proof-key-hex <hex>|--new-password-env <env> [--new-super-salt-hex <hex>]) [--rotation-generation <id>] <target ...> [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | agent-update --artifact-url <https-url> --sha256-hex <sha256> [--artifact-signature-hex <sig>] [--artifact-signing-key-hex <key>] <target ...> [--timeout <1-3600>] [--proof-ttl <1-3600>] [--force-unprivileged] --confirmed | agent-update-activate --staged-sha256-hex <sha256> <target ...> [--timeout <1-3600>] [--proof-ttl <1-3600>] [--force-unprivileged] --confirmed | agent-update-rollback [--rollback-sha256-hex <sha256>] <target ...> [--timeout <1-3600>] [--proof-ttl <1-3600>] [--force-unprivileged] --confirmed | agent-update-rollout-policies [--limit <1-200>] [--enabled true|false] [--channel <name>] | agent-update-rollout-policy-create --name <name> --scope-kind global|tag|pool|provider [--scope-value <value>] [--channel <name>] [--canary-count <0-10000>] [--health-gate heartbeat_verified|manual_after_canary|manual_only] [--priority <n>] [--disabled] --confirmed | agent-update-releases [--limit <1-200>] | agent-update-release-latest [--name <name>] [--channel stable] | agent-update-release-record --name <name> --version <version> --artifact-url <https-url> --sha256-hex <sha256> --artifact-signature-hex <sig> --artifact-signing-key-hex <key> [--rollback-artifact-file <path> --rollback-artifact-url <https-url> --rollback-signing-seed-hex <seed>] [--channel stable] [--size-bytes <n>] [--note <text>] --confirmed | agent-update-artifact-upload --name <name> --version <version> --artifact-file <path> --signing-seed-hex <seed> [--rollback-artifact-file <path>] [--rollback-signing-seed-hex <seed>] [--stream] [--channel stable] [--note <text>] --confirmed | process-list <target ...> [--limit <1-512>] [--confirmed] | process-start <name> --argv <abs> [--argv arg ...] <target ...> [--cwd <abs>] [--env KEY=VALUE] [--confirmed] | process-stop <name> <target ...> [--confirmed] | process-restart <name> <target ...> [--confirmed] | process-status [--name <name>] <target ...> [--confirmed] | process-logs <name> <target ...> [--max-bytes <n>] [--confirmed] | process-supervisor-inventory [--limit <1-200>] | job-targets <job_uuid> | job-outputs <job_uuid> | job-follow <job_uuid> [--interval-ms <100-10000>] [--max-polls <1-10000>] [--json] | job-output-artifact <job_uuid> <client_id> --seq <seq> <output_file> | backups | backup-policies | backup-artifacts | backup-policy-upsert <name> [--path <abs>] [--include-config] [--recipient-public-key-hex <hex>] [--interval <secs>] [--retention-days <n>] [--keep-last <n>] [--rotation-generation <id>] [--disabled] <target ...> --confirmed | backup-policy-prune [--schedule-id <uuid>] [--dry-run] [--metadata-only true|false] [--confirmed] | backup-request <client_id> [--path <abs>] [--include-config] [--confirmed] | backup-run [--path <abs>] [--include-config] <target ...> [--timeout <1-3600>] --confirmed | backup-artifact-record <backup_uuid> --object-key <key> --sha256-hex <sha256> --size-bytes <n> --confirmed | backup-artifact-upload <backup_uuid> --object-key <key> --artifact-file <path> --confirmed | backup-artifact-upload-chunked <backup_uuid> --object-key <key> --artifact-file <path> [--chunk-size-bytes <1-4194304>] --confirmed | backup-artifact-handoff <backup_uuid> [--job-id <job_uuid>] --confirmed | restore-plans | restore-plan <backup_uuid> <target_client_id> [--path <abs>] [--include-config] [--destination-root <abs>] [--confirmed] | restore-run <backup_uuid> <target_client_id> [--artifact-file <path>] [--private-key-env <env>] [--path <abs>] [--include-config] [--destination-root <abs>] [--timeout <1-3600>] [--force-unprivileged] --confirmed | restore-rollback <restore_job_uuid> <target_client_id> [--timeout <1-3600>] [--force-unprivileged] --confirmed | migration-links | migration-link <restore_plan_uuid> [--note <text>] --confirmed | migration-run <restore_plan_uuid> [--artifact-file <path>] [--private-key-env <env>] [--note <text>] [--timeout <1-3600>] [--force-unprivileged] --confirmed | tunnel-plans | tunnel-plan --name <name> --interface-name <ifname> --kind <gre|ipip|sit|fou|openvpn|wireguard|tun_tap|custom> --left-client-id <id> --right-client-id <id> --left-underlay <ip> --right-underlay <ip> --address-pool-cidr <cidr> --bandwidth <10m|100m|1000m> --latency-ms <ms> [--runtime-manager <agent|observed|adapter>] [--runtime-cleanup-argv <abs,arg>] [--fou-port <1-65535>] [--fou-peer-port <1-65535>] [--fou-ipproto <1-255>] [--save] | tunnel-promote-telemetry --client-id <id> --interface <ifname> --peer-client-id <id> --local-underlay <ip> --peer-underlay <ip> --address-pool-cidr <cidr> [--side <left|right>] [--bandwidth <10m|100m|1000m>] | tunnel-promote-adapter --plan-id <uuid> --runtime-status-argv <abs,arg> [--runtime-startup-argv <abs,arg>] [--runtime-stop-argv <abs,arg>] [--runtime-cleanup-argv <abs,arg>] [--fou-port <1-65535>] [--fou-peer-port <1-65535>] [--fou-ipproto <1-255>] --confirmed | tunnel-apply --plan-file <plan.json> --side <left|right> [--backend <ifupdown|netplan|systemd-networkd>] [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | tunnel-ospf-cost-update --plan-file <plan.json> --side <left|right> --current-ospf-cost <1-65535> --recommended-ospf-cost <1-65535> [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | tunnel-rollback --plan-file <plan.json> --side <left|right> [--timeout <1-3600>] [--proof-ttl <1-3600>] --confirmed | tunnel-status --plan-file <plan.json> --side <left|right> [--timeout <1-3600>] [--proof-ttl <1-3600>] | tunnel-probe --plan-file <plan.json> --side <left|right> [--count <1-20>] [--interval-ms <200-10000>] [--timeout <1-3600>] [--proof-ttl <1-3600>] | tunnel-speed-test --plan-file <plan.json> --server-side <left|right> [--duration-secs <1-30>] [--max-bytes <16384-268435456>] [--rate-limit-kbps <64-1000000>] [--port <1024-65535>] [--connect-timeout-ms <100-30000>] [--timeout <1-3600>] [--proof-ttl <1-3600>] | network-observations [--limit <1-200>] | network-trends [--limit <1-200>] | network-ospf-recommendations [--limit <1-200>] | network-ospf-update-plans [--limit <1-200>] | topology-graph [--limit <1-200>] | audit | history-retention | history-retention-upsert --domain <domain> [--retention-days <1-3650>] [--prune-limit <1-100000>] [--metadata-only true|false] --confirmed | history-retention-prune [--domain <domain>] [--dry-run] [--metadata-only true|false] [--confirmed] | history-export [--domains audit_logs,job_outputs] [--limit <1-200>] | bulk-resolve [tag ...] | enable | proof | signing-keygen | exit"
                )
            }
