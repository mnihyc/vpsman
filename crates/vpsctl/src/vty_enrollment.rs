...
fn parse_vty_enrollment_token_create(tokens: &[&str]) -> Result<VtyEnrollmentTokenCreate> {
    ...
    parse_common_flags(tokens, &mut request)?;
    if request.purpose == "rebuild_reenrollment" && !request.confirmed_reenrollment {
        bail!("rebuild re-enrollment token requires --confirmed");
    }
    Ok(request)
}
...
fn parse_vty_reenrollment_token_create(tokens: &[&str]) -> Result<VtyEnrollmentTokenCreate> {
    ...
    parse_common_flags(tokens, &mut request)?;
    if request.allowed_client_id.is_none() {
...
fn parse_common_flags(tokens: &[&str], request: &mut VtyEnrollmentTokenCreate) -> Result<()> {
    let mut index = 0;
    while index < tokens.len() {
        match tokens[index] {
            ...
            "--purpose" => {
                request.purpose = parse_next(tokens, &mut index, "purpose")?.to_string();
            }
            "--allowed-client-id" | "--client-id" => {
                request.allowed_client_id =
                    Some(parse_next(tokens, &mut index, "client_id")?.to_string());
            }
            "--tags" | "--default-tags" => {
...
mod tests {
    ...
    fn parses_vty_reenrollment_token_create() {
        ...
        assert_eq!(request.allowed_client_id.as_deref(), Some("edge-a"));
    ...
    fn rejects_unconfirmed_vty_reenrollment_token() {
        ...
        assert!(parse_vty_enrollment_token_create(&[
            "--purpose",
            "rebuild_reenrollment",
            "--client-id",
            "edge-a"
        ])
        .is_err());
