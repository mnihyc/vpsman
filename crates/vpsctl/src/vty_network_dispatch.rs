        ...
        "tunnel-status" => {
            ...
            let request = match parse_vty_tunnel_status(&parts[1..]) {
                ...
                Err(error) => {
                    println!("usage error: {error}");
                    println!(
                        "usage: tunnel-status --plan-file <plan.json> --side <left|right> [--max-timeout <secs>] [--privilege-ttl <15-300>]"
                    );
                    return Ok(());
                }
            };
        ...
        "tunnel-probe" => {
            ...
            let request = match parse_vty_tunnel_probe(&parts[1..]) {
                ...
                Err(error) => {
                    println!("usage error: {error}");
                    println!(
                        "usage: tunnel-probe --plan-file <plan.json> --side <left|right> [--count <1-20>] [--interval-ms <200-10000>] [--max-timeout <secs>] [--privilege-ttl <15-300>]"
                    );
                    return Ok(());
                }
            };
        ...
        "tunnel-speed-test" => {
            ...
            let request = match parse_vty_tunnel_speed_test(&parts[1..]) {
                ...
                Err(error) => {
                    println!("usage error: {error}");
                    println!(
                        "usage: tunnel-speed-test --plan-file <plan.json> --server-side <left|right> [--duration-secs <1-30>] [--max-bytes <16384-268435456>] [--rate-limit-kbps <64-1000000>] [--port <1024-65535>] [--connect-timeout-ms <100-30000>] [--max-timeout <secs>] [--privilege-ttl <15-300>] --confirmed"
                    );
                    return Ok(());
                }
            };
