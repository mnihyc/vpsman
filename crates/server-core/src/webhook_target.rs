...
mod tests {
    ...
    fn secure_policy_requires_public_https_without_credentials() {
        assert!(
            validate_webhook_target_with_policy("https://hooks.acme.com/vpsman", secure()).is_ok()
        );
        for target in [
    ...
    fn resolution_rejects_mixed_or_non_public_answers_without_network() {
        let url = validate_webhook_target_with_policy("https://hooks.acme.com:8443/hook", secure())
            .unwrap();
        ...
        assert!(
            validate_resolved_addresses(&url, [public, private], AddressRequirement::Public)
                .is_err()
        );
    ...
    fn development_hostname_resolution_must_remain_loopback() {
        let url = validate_webhook_target_with_policy("http://localhost:9000/hook", development())
            .unwrap();
