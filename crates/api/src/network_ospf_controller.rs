...
pub(crate) async fn run_controller_sweep(state: &AppState) -> Result<usize> {
    ...
    for update in update_plans
        .into_iter()
        .filter(|plan| plan.control_mode == "automatic")
    {
        let Some(plan) = state.repo.get_tunnel_plan(update.plan_id).await? else {
            continue;
        };
...
async fn reconcile_orphaned_pending_jobs(state: &AppState) -> Result<()> {
    ...
    for plan in state
        ...
        .await?
        .into_iter()
        .filter(|plan| plan.ospf_status == "pending")
    {
