...
mod tests {
    ...
    #[test]
    fn terminal_rollup_frontiers_share_one_tier_aware_index_contract() {
        assert_eq!(
            NETWORK_OBSERVATION_BUCKET_SECS,
            [60, 300, 1_800, 3_600, 10_800, 21_600, 86_400]
        );
        let source = include_str!("network_observation_retention.rs");
        let production = source.rsplit_once("#[cfg(test)]\nmod tests").unwrap().0;
        let exact_width = ["rollup.bucket_secs = ", "tier.bucket_secs"].concat();
        assert_eq!(
            production
                .matches("FROM unnest($2::integer[]) AS tier(bucket_secs)")
                .count(),
            2
        );
        assert_eq!(
            production
                .matches("FROM unnest($3::integer[]) AS tier(bucket_secs)")
                .count(),
            1
        );
        assert_eq!(production.matches(&exact_width).count(), 3);
    ...
    #[test]
    fn automatic_locators_have_one_canonical_sample_lifecycle_owner() {
        let source = include_str!("network_observation_retention.rs");
        let readiness = source
            ...
            .unwrap()
            .1
            .split_once("pub(crate) async fn network_observation_retention_phase_next_at(")
            .unwrap()
            .0;
        let next_at = source
            .split_once("pub(crate) async fn network_observation_retention_phase_next_at(")
            .unwrap()
            .1
            .split_once("fn add_promotion_result(")
            .unwrap()
            .0;
        let claim = source
            .split_once("async fn prune_expired_exact_observations(")
            .unwrap()
            .1
            .split_once("async fn prune_expired_rollups(")
            .unwrap()
            .0;
        for terminal_path in [readiness, next_at, claim] {
            assert!(terminal_path.contains("observation.source = 'manual'"));
            assert!(!terminal_path.contains("observation.source = 'automatic'"));
            assert!(!terminal_path.contains("telemetry_minute_materialization_heads"));
        }

        let raw_owner = include_str!("history_retention.rs");
        let sample_owner = raw_owner
            .split_once("fn sample_prune_owner_query()")
            .unwrap()
            .1
            .split_once("fn sample_prune_query()")
            .unwrap()
            .0;
        let sample_prune = raw_owner
            .split_once("fn sample_prune_query()")
            .unwrap()
            .1
            .split_once("fn prune_query(")
            .unwrap()
            .0;
        for consumer in [
            "telemetry_projection_heads",
            "telemetry_webhook_cursors",
            "telemetry_minute_materialization_heads",
            "traffic_counter_minute_heads",
        ] {
            assert!(sample_owner.contains(consumer));
            assert!(!sample_prune.contains(consumer));
        }
        assert!(sample_prune.contains("DELETE FROM telemetry_samples"));

        let migration = include_str!("../../../../migrations/0004_network_tunnels.sql");
        assert!(migration.contains(
            "FOREIGN KEY (automatic_sample_id) REFERENCES public.telemetry_samples(id) ON DELETE CASCADE"
        ));
        assert!(migration.contains(
        ...
        let ingress = include_str!("../../../api/src/routes/fleet/routes_ingest.rs");
        assert!(ingress
            .contains("saturating_sub(observation.measured_unix)\n                    > 3_900"));
        assert!(ingress.contains("measured_unix > metrics.observed_unix.saturating_add(300)"));
    }

    #[test]
    fn rollup_due_edges_use_the_exact_primary_key_ordering_contract() {
        let source = include_str!("network_observation_retention.rs");
        let due_probe = source
            .split_once("async fn observation_rollup_promotion_is_due(")
            .unwrap()
            .1
            .split_once("fn observation_rollup_source_bucket_secs(")
            .unwrap()
            .0;
        let claim = source
            .split_once("async fn promote_rollups(")
            .unwrap()
            .1
            .split_once("fn promotion_result_from_row(")
            .unwrap()
            .0;
        let next_at = source
            .split_once("pub(crate) async fn network_observation_retention_phase_next_at(")
            ...
            .unwrap()
            .0;
        for edge_path in [due_probe, claim] {
            assert!(edge_path.contains("due.destination_start <= now()"));
            assert!(edge_path.contains("ORDER BY due.destination_start, due.owner_identity"));
            assert!(!edge_path.contains("due.due_at"));
        }
        assert!(next_at.contains("ORDER BY destination_start, owner_identity"));
        assert!(!next_at.contains("ORDER BY due_at"));
        ...
        assert!(migration.contains("CONSTRAINT telemetry_history_due_spans_due_at_check CHECK ("));
    }
    ...
    #[test]
    fn promotion_is_exact_span_setwise_additive_and_uncapped() {
        assert!(ROLLUP_PROMOTION_QUERY.contains("eligible_source AS MATERIALIZED"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("source.series_id = $5"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("source.bucket_secs = $1"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("AS expected_source_rows"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("AS locked_source_rows"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("locked_source_rows >= expected_source_rows"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("AS inserted_destination_rows"));
        assert!(ROLLUP_PROMOTION_QUERY
            .contains("inserted_destination_rows >= expected_destination_rows"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("span_coverage AS MATERIALIZED"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("telemetry_history_due_spans"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("due.owner_identity = ARRAY[$5::bigint::text]"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("FOR NO KEY UPDATE OF series SKIP LOCKED"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("FOR UPDATE OF source SKIP LOCKED"));
        assert!(ROLLUP_PROMOTION_QUERY.contains("DO UPDATE SET"));
        assert!(ROLLUP_PROMOTION_QUERY
            .contains("sample_count = current.sample_count + EXCLUDED.sample_count"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("LATERAL"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("LIMIT"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("history_retention_scan_cursors"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("eligible_groups AS MATERIALIZED"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("complete_groups AS MATERIALIZED"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("fully_inserted_groups AS MATERIALIZED"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("destination_conflicts"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("overlapping_groups"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("interval '2 hours'"));
        assert!(!ROLLUP_PROMOTION_QUERY.contains("promote_from"));
    }
